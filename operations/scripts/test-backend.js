...
const getLocalDbPool = () => {
  if (!localDbPool) {
    ({ pool: localDbPool } = require(path.join(workspaceRoot, 'backend', 'config', 'db')));
  }
...
const assertSchemaRejected = ({ response, payload }, context) => {
  assertStatus(response, payload, 400, context);
  if (payload?.code !== 'REQUEST_SCHEMA_INVALID') {
    throw new Error(`${context} returned unexpected code: ${payload?.code || 'missing'}.`);
  }
};

const runPublicRequestSchemaSmoke = async () => {
  const malformedBody = await fetchJson('/api/auth/login', {
    method: 'POST',
    body: {
  ...
  assertSchemaRejected(malformedBody, 'Reject malformed public request body');

  const malformedQuery = await fetchJson('/api/auth/config?unexpected=1');
  assertSchemaRejected(malformedQuery, 'Reject malformed public request query');

  const malformedParam = await fetchJson('/api/members/not-a-number');
  assertSchemaRejected(malformedParam, 'Reject malformed public route parameter');

  console.log('Public HTTP request schema smoke passed.');
};
...
const runAuthenticatedPaymentMembershipSmoke = async (auth) => {
  ...
  const unknownPaymentField = await fetchJson('/api/payments/record', {
    method: 'POST',
    headers,
    body: {
      user_id: fixtures.memberId,
      ...
      total_amount: 1000,
      payment_mode: 'Cash',
      unexpected_gateway_override: true,
    },
  });
  assertStatus(unknownPaymentField.response, unknownPaymentField.payload, 400, 'Reject unknown payment field');
  if (unknownPaymentField.payload?.code !== 'REQUEST_SCHEMA_INVALID') {
    throw new Error(`Reject unknown payment field returned unexpected code: ${unknownPaymentField.payload?.code || 'missing'}.`);
  }
...
const syntaxTargets = [
  ...
  ...collectJsFiles(path.join(workspaceRoot, 'backend')),
  ...collectJsFiles(path.join(workspaceRoot, 'database')),
  ...collectJsFiles(path.join(workspaceRoot, 'operations', 'jobs')),
  ...collectJsFiles(path.join(workspaceRoot, 'operations', 'scripts')).filter((filePath) => !filePath.endsWith('test-backend.js')),
];

const uniqueTargets = Array.from(new Set(syntaxTargets));

const testNoRawServerErrorResponses = () => {
  const rawServerErrorPattern = /status\(\s*5\d\d\s*\)\.json\(\s*\{\s*(?:error|message)\s*:\s*(?:err|error)(?:\?\.)?\.message/;
  const violations = uniqueTargets
    .filter((target) => rawServerErrorPattern.test(fs.readFileSync(target, 'utf8')))
    .map((target) => path.relative(workspaceRoot, target));

  assert.deepEqual(
    violations,
    [],
    `Raw error messages must not be returned by 5xx responses: ${violations.join(', ')}`
  );
};
...
const testTelemetryRedaction = () => {
  ...
  const sanitized = sanitizeTelemetryValue({
    ...
    user_agent: 'GymVault smoke client',
  });
  ...
  assert.match(sanitized.nested.note, /token=\[REDACTED\]/i);
  assert.equal(sanitized.nested.jwt, '[REDACTED_JWT]');
  assert.equal(sanitized.user_agent, 'GymVault smoke client');
  assert.doesNotMatch(serialized, /owner@example\.com|98765|super-secret-value|abcdefghij\./i);
};

const testTenantDbContextIsolation = async () => {
  const { getTenantDbContext, runWithTenantDbContext } = require('../../backend/utils/tenantDbContext');
  const observed = await Promise.all([
    runWithTenantDbContext({ gymId: 11, actorId: 101, actorRole: 'owner' }, async () => {
      await Promise.resolve();
      return getTenantDbContext();
    }),
    runWithTenantDbContext({ gymId: 22, actorId: 202, actorRole: 'member' }, async () => {
  ...
  assert.deepEqual(observed, [
    { gymId: 11, actorId: 101, actorRole: 'OWNER' },
    { gymId: 22, actorId: 202, actorRole: 'MEMBER' },
  ]);
  assert.equal(getTenantDbContext(), null);
};
