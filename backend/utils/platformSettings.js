...
const normalizeBillingPlan = (planId, value) => {
    ...
    const raw = value && typeof value === 'object' ? value : {};
    const normalizedLimits = normalizePlanLimits(raw.limits, defaults.limits);
    return {
        id: planId,
...
const ensurePlatformSettingsBase = async () => {
    if (!ensurePlatformSettingsBasePromise) {
        ensurePlatformSettingsBasePromise = adminPool.query(`
...
const ensureGymBillingAddonSchema = async () => {
    if (!ensureGymBillingAddonSchemaPromise) {
        ensureGymBillingAddonSchemaPromise = adminPool.query(`
            ALTER TABLE gyms
