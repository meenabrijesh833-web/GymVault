...
const MAX_PROFILE_IMAGE_DIMENSION = 2048;
...
const sanitizeProfileImageBuffer = async (buffer, extension) => {
    ...
    if (sanitized.length > MAX_PROFILE_IMAGE_BYTES) {
        throw new Error('Sanitized image exceeds the profile image size limit.');
    }
    return sanitized;
};
...
const respondToUploadError = (req, res, err) => {
    void recordSecurityEvent(req, {
        eventType: 'UPLOAD_REJECTED',
        message: 'Profile image upload was rejected.',
        statusCode: err instanceof multer.MulterError || err ? 400 : 500,
        metadata: { reason: err?.code || err?.name || 'invalid_upload' },
    });
...
const createProfileUploadMiddleware = ({ prefix = 'profile', getActorId, storageMode = 'disk' } = {}) => {
    return (req, res, next) => {
        baseUpload.single('profile_pic')(req, res, async (err) => {
            if (err) {
                return respondToUploadError(req, res, err);
            }
            ...
            try {
                ...
                if (!detectedExtension || (expectedExtension && expectedExtension !== detectedExtension)) {
                    void recordSecurityEvent(req, {
                        eventType: 'UPLOAD_REJECTED',
                        message: 'Profile image content did not match its declared type.',
                        statusCode: 400,
                        metadata: { reason: 'mime_mismatch' },
                    });
                    return res.status(400).json({ error: 'Uploaded file content does not match a supported image type.' });
                }

                try {
                    req.file.buffer = await sanitizeProfileImageBuffer(req.file.buffer, detectedExtension);
                    req.file.size = req.file.buffer.length;
                } catch (_imageError) {
                    void recordSecurityEvent(req, {
                        eventType: 'UPLOAD_REJECTED',
                        message: 'Profile image decoder rejected the upload.',
                        statusCode: 400,
                        metadata: { reason: 'decode_failed' },
                    });
                    return res.status(400).json({ error: 'Image could not be safely decoded. Choose a valid JPG, PNG, or WEBP file.' });
                }
