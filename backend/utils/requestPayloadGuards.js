...
const DEFAULT_NESTING_LIMIT = 12;
const DEFAULT_KEY_LENGTH_LIMIT = 80;
...
const inspectValue = (value, path, depth = 0) => {
    ...
    if (depth > DEFAULT_NESTING_LIMIT) {
        throw new Error(`Field ${path.replace(/^body\./, '')} exceeds the nesting limit.`);
    }
    ...
    if (Array.isArray(value)) {
        ...
        value.forEach((entry, index) => inspectValue(entry, `${path}[${index}]`, depth + 1));
        return;
    }
    ...
    if (typeof value === 'object') {
        ...
        keys.forEach((key) => {
            if (BLOCKED_OBJECT_KEYS.has(key)) {
                throw new Error('Request payload contains a prohibited object key.');
            }
            if (key.length > DEFAULT_KEY_LENGTH_LIMIT) {
                throw new Error(`Field ${path.replace(/^body\./, '')} contains an oversized key.`);
            }
            inspectValue(value[key], `${path}.${key}`, depth + 1);
        });
    }
};

const recordPayloadRejection = (req, error) => {
    try {
        const { recordRuntimeEvent } = require('./runtimeTelemetry');
        void recordRuntimeEvent({
            eventType: 'PAYLOAD_GUARD_REJECT',
            severity: 'WARN',
            source: 'security',
            message: `${req.method} ${req.originalUrl || req.path || '/'} rejected by payload guard`,
            ...
            metadata: {
                reason: String(error?.message || 'invalid_payload').slice(0, 300),
                client_ip: String(req.ip || req.socket?.remoteAddress || '').slice(0, 80),
            },
        });
    } catch (_error) {
        // Payload rejection must never be blocked by telemetry failures.
    }
};
...
const enforceRequestPayloadLimits = (req, res, next) => {
    ...
    try {
        ...
        recordPayloadRejection(req, err);
