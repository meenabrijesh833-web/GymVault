...
const clientErrorBody = strictBody(['scope', 'message', 'error', 'stack', 'page', 'extra', 'componentStack'], {
    fieldRules: {
...
const supportTicketBody = strictBody(['subject', 'category', 'priority', 'description'], {
    requiredFields: ['subject', 'description'],
    fieldRules: {
        subject: { type: 'string', minLength: 1, maxLength: 200 },
        category: { type: 'string', enum: ['GENERAL', 'ACCOUNT', 'BILLING', 'TECHNICAL', 'DATA'], caseInsensitive: true },
        priority: { type: 'string', enum: ['LOW', 'MEDIUM', 'HIGH'], caseInsensitive: true },
        description: { type: 'string', minLength: 1, maxLength: 5000 },
    },
});
const supportMessageBody = strictBody(['message'], {
    requiredFields: ['message'],
    fieldRules: { message: { type: 'string', minLength: 1, maxLength: 5000 } },
});
const chatbotMessageBody = strictBody(['message'], {
    requiredFields: ['message'],
    fieldRules: { message: { type: 'string', minLength: 1, maxLength: 1000 } },
});

router.post('/client-errors', clientErrorBody, async (req, res) => {
    try {
...
router.post('/tickets', requirePermission('support:write'), supportTicketBody, async (req, res) => {
...
router.post('/tickets/:id/messages', requirePermission('support:write'), supportMessageBody, async (req, res) => {
    const ticketId = parseInt(req.params.id, 10);
...
router.post('/chatbot', requirePermission('support:read'), chatbotMessageBody, async (req, res) => {
