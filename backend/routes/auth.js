...
const recordAuthFailure = (req, flow, reason, statusCode, metadata = {}) => {
    void recordSecurityEvent(req, {
        eventType: 'AUTH_FAILED',
        message: 'Authentication attempt was rejected.',
        statusCode,
        metadata: { flow, reason, ...metadata },
    });
};
...
const REQUIRED_EMAIL_BODY_SCHEMA = {
    requiredFields: ['email'],
    fieldRules: {
        email: { type: 'string', format: 'email', maxLength: 254 },
    },
};
const REQUIRED_PHONE_BODY_SCHEMA = {
    requiredFields: ['phone'],
    fieldRules: {
        phone: { type: 'string', format: 'phone', maxLength: 30 },
    },
};
const REQUIRED_EMAIL_OTP_BODY_SCHEMA = {
...
const REQUIRED_PHONE_OTP_BODY_SCHEMA = {
...
const handleMemberVerifyOtp = async (req, res) => {
    ...
    try {
        ...
        if (!otpRow) {
            recordAuthFailure(req, 'member_email_otp', 'missing_or_expired', 400);
        ...
        if (previousAttempts >= MEMBER_LOGIN_MAX_VERIFY_ATTEMPTS) {
            ...
            recordAuthFailure(req, 'member_email_otp', 'attempt_limit', 429, { attempts: previousAttempts });
        ...
        if (!isMatch) {
            ...
            recordAuthFailure(req, 'member_email_otp', shouldLock ? 'attempt_limit' : 'invalid_code', shouldLock ? 429 : 400, { attempts: nextAttempts });
...
router.post('/check-email', strictBody(['email'], REQUIRED_EMAIL_BODY_SCHEMA), async (req, res) => {
...
router.post('/signup/send-email-otp', strictBody(['email'], REQUIRED_EMAIL_BODY_SCHEMA), async (req, res) => {
    const email = String(req.body?.email || '').trim().toLowerCase();
...
router.post('/signup/verify-email-otp', strictBody(['email', 'otp'], REQUIRED_EMAIL_OTP_BODY_SCHEMA), async (req, res) => {
    const email = String(req.body?.email || '').trim().toLowerCase();
    const otp = String(req.body?.otp || '').replace(/\D/g, '').slice(0, 6);
    ...
    try {
        ...
        const otpRow = activeOtp.rows[0];
        if (!otpRow) {
            recordAuthFailure(req, 'signup_email_otp', 'missing_or_expired', 400);
        ...
        if (previousAttempts >= PASSWORD_RESET_MAX_VERIFY_ATTEMPTS) {
            ...
            recordAuthFailure(req, 'signup_email_otp', 'attempt_limit', 429, { attempts: previousAttempts });
            return res.status(429).json({ message: `Too many invalid attempts. Try again in ${OTP_LOCKOUT_MINUTES} minutes.`, retry_after_seconds: OTP_LOCKOUT_MINUTES * 60 });
        }
        ...
        if (!isMatch) {
            ...
            recordAuthFailure(req, 'signup_email_otp', shouldLock ? 'attempt_limit' : 'invalid_code', shouldLock ? 429 : 400, { attempts: nextAttempts });
...
router.post('/check-phone', strictBody(['phone'], REQUIRED_PHONE_BODY_SCHEMA), async (req, res) => {
...
router.post('/register-owner', strictBody([
    'gym_name',
    'full_name',
    'email',
    'password',
    ...
    fieldRules: {
        gym_name: { type: 'string', minLength: 1, maxLength: 100 },
        full_name: { type: 'string', minLength: 1, maxLength: 100 },
        email: { type: 'string', format: 'email', maxLength: 254 },
        password: { type: 'string', minLength: 8, maxLength: 256 },
        email_verification_token: { type: 'string', minLength: 1, maxLength: 4096 },
        owner_phone: { type: 'string', format: 'phone', maxLength: 30 },
        gym_address: { type: 'string', maxLength: 500 },
        gym_city: { type: 'string', maxLength: 120 },
        branches_count: { type: 'integer', min: 1, max: 25 },
        selected_plan: { type: 'string', enum: ['basic', 'growth', 'pro', 'elite'], caseInsensitive: true },
        legal_acceptance: { type: 'boolean', enum: [true] },
        terms_version: { type: 'string', enum: [TERMS_VERSION] },
        privacy_version: { type: 'string', enum: [PRIVACY_VERSION] },
    },
}), async (req, res) => {
    ...
    if (!validateLegalAcceptance(req.body)) {
        return res.status(400).json({
            ...
            message: 'Accept the current Terms of Service and Privacy Policy to create an account.',
        });
    }
...
router.post('/login', strictBody(['email', 'password'], {
    ...
    try {
        ...
        if (userResult.rows.length === 0) {
            recordAuthFailure(req, 'password', 'account_not_found', 400);
        ...
        if (!isMatch) {
            recordAuthFailure(req, 'password', 'invalid_password', 400);
            return res.status(400).json({ message: "Invalid email or password." });
        }
...
router.post('/admin/send-otp', strictBody(['phone'], REQUIRED_PHONE_BODY_SCHEMA), async (req, res) => {
    const phone = normalizeLocalIndianPhone(req.body?.phone);
...
router.post('/admin/verify-otp', strictBody(['phone', 'otp'], REQUIRED_PHONE_OTP_BODY_SCHEMA), async (req, res) => {
    const phone = normalizeLocalIndianPhone(req.body?.phone);
    const otp = String(req.body?.otp || '').replace(/\D/g, '').slice(0, 6);
    ...
    try {
        ...
        const user = await loadAdminAuthContextByPhone(phone);
        if (!user) {
            recordAuthFailure(req, 'admin_phone_otp', 'account_not_found', 400);
        ...
        const otpRow = activeOtp.rows[0];
        if (!otpRow) {
            recordAuthFailure(req, 'admin_phone_otp', 'missing_or_expired', 400);
            return res.status(400).json({ message: 'The OTP is invalid or expired. Request a new one.' });
        }
        ...
        if (previousAttempts >= ADMIN_LOGIN_MAX_VERIFY_ATTEMPTS) {
            ...
            recordAuthFailure(req, 'admin_phone_otp', 'attempt_limit', 429, { attempts: previousAttempts });
            return res.status(429).json({ message: `Too many invalid attempts. Try again in ${OTP_LOCKOUT_MINUTES} minutes.`, retry_after_seconds: OTP_LOCKOUT_MINUTES * 60 });
        }
        ...
        if (!isValidOtp) {
            ...
            recordAuthFailure(req, 'admin_phone_otp', shouldConsumeOtp ? 'expired' : (shouldLock ? 'attempt_limit' : 'invalid_code'), shouldLock ? 429 : 400, { attempts: nextAttempts });
...
router.post('/admin/send-email-otp', strictBody(['email'], REQUIRED_EMAIL_BODY_SCHEMA), async (req, res) => {
    const email = String(req.body?.email || '').trim().toLowerCase();
...
router.post('/admin/verify-email-otp', strictBody(['email', 'otp'], REQUIRED_EMAIL_OTP_BODY_SCHEMA), async (req, res) => {
    const email = String(req.body?.email || '').trim().toLowerCase();
    const otp = String(req.body?.otp || '').replace(/\D/g, '').slice(0, 6);
    ...
    try {
        ...
        const user = await loadAdminAuthContextByEmail(email);
        if (!user) {
            recordAuthFailure(req, 'admin_email_otp', 'account_not_found', 400);
            return res.status(400).json({ message: 'The OTP is invalid or expired. Request a new one.' });
        }
        ...
        const otpRow = activeOtp.rows[0];
        if (!otpRow) {
            recordAuthFailure(req, 'admin_email_otp', 'missing_or_expired', 400);
            return res.status(400).json({ message: 'The OTP is invalid or expired. Request a new one.' });
        }
        ...
        if (previousAttempts >= ADMIN_LOGIN_MAX_VERIFY_ATTEMPTS) {
            ...
            recordAuthFailure(req, 'admin_email_otp', 'attempt_limit', 429, { attempts: previousAttempts });
            return res.status(429).json({ message: `Too many invalid attempts. Try again in ${OTP_LOCKOUT_MINUTES} minutes.`, retry_after_seconds: OTP_LOCKOUT_MINUTES * 60 });
        }
        ...
        if (!isMatch) {
            ...
            recordAuthFailure(req, 'admin_email_otp', shouldLock ? 'attempt_limit' : 'invalid_code', shouldLock ? 429 : 400, { attempts: nextAttempts });
...
router.post('/password-reset/request', strictBody(['email'], REQUIRED_EMAIL_BODY_SCHEMA), async (req, res) => {
    const email = String(req.body?.email || '').trim().toLowerCase();
...
router.post('/password-reset/confirm', strictBody(['email', 'otp', 'new_password'], {
    ...
    try {
        const user = await loadUserAuthContextByEmail(email);
        if (!user) {
            recordAuthFailure(req, 'password_reset_otp', 'account_not_found', 400);
        ...
        if (!resetRow) {
            recordAuthFailure(req, 'password_reset_otp', 'missing_or_expired', 400);
            return res.status(400).json({ message: 'The reset code is invalid or expired. Request a new one.' });
        }
        ...
        if (previousAttempts >= PASSWORD_RESET_MAX_VERIFY_ATTEMPTS) {
            ...
            recordAuthFailure(req, 'password_reset_otp', 'attempt_limit', 429, { attempts: previousAttempts });
            return res.status(429).json({ message: `Too many invalid attempts. Try again in ${OTP_LOCKOUT_MINUTES} minutes.`, retry_after_seconds: OTP_LOCKOUT_MINUTES * 60 });
        }
        ...
        if (!isMatch) {
            ...
            recordAuthFailure(req, 'password_reset_otp', shouldLock ? 'attempt_limit' : 'invalid_code', shouldLock ? 429 : 400, { attempts: nextAttempts });
...
router.post('/oauth/bootstrap', strictBody(['bootstrap_token'], {
    requiredFields: ['bootstrap_token'],
    fieldRules: {
        bootstrap_token: { type: 'string', minLength: 1, maxLength: 8192 },
    },
}), async (req, res) => {
...
router.post('/google/signup/complete', strictBody([
    'signup_token',
    'gym_name',
    'full_name',
    'owner_phone',
    ...
    fieldRules: {
        signup_token: { type: 'string', minLength: 1, maxLength: 8192 },
        gym_name: { type: 'string', minLength: 1, maxLength: 100 },
        full_name: { type: 'string', minLength: 1, maxLength: 100 },
        owner_phone: { type: 'string', format: 'phone', maxLength: 30 },
        gym_address: { type: 'string', maxLength: 500 },
        gym_city: { type: 'string', minLength: 1, maxLength: 120 },
        branches_count: { type: 'integer', min: 1, max: 25 },
        selected_plan: { type: 'string', enum: ['basic', 'growth', 'pro', 'elite'], caseInsensitive: true },
        legal_acceptance: { type: 'boolean', enum: [true] },
        terms_version: { type: 'string', enum: [TERMS_VERSION] },
...
router.post('/apple', strictBody(['id_token', 'full_name'], {
    requiredFields: ['id_token'],
    fieldRules: {
        id_token: { type: 'string', minLength: 1, maxLength: 16384 },
...
router.post('/member/profiles', strictBody(['phone'], REQUIRED_PHONE_BODY_SCHEMA), handleMemberProfileLookup);
router.post('/member/send-otp', strictBody(['phone', 'member_id'], {
    requiredFields: ['phone'],
    fieldRules: {
        phone: { type: 'string', format: 'phone', maxLength: 30 },
...
router.post('/member/verify-otp', strictBody(['phone', 'member_id', 'otp'], {
    requiredFields: ['phone', 'otp'],
    fieldRules: {
        phone: { type: 'string', format: 'phone', maxLength: 30 },
        member_id: { type: 'integer', min: 1 },
...
}), handleMemberVerifyOtp);

router.post('/logout', strictBody([], { allowEmpty: true }), (_req, res) => {
