...
const attendanceModeBody = strictBody([
    'attendance_mode',
    'attendance_geo_enabled',
    'gym_latitude',
    'gym_longitude',
    'gym_radius_meters',
    'allow_expired_checkin',
], {
    fieldRules: {
        attendance_mode: { type: 'string', enum: ['STAFF', 'QR', 'SELF', 'RFID'], caseInsensitive: true },
        attendance_geo_enabled: { type: 'boolean' },
        gym_latitude: { type: 'number', min: -90, max: 90 },
        gym_longitude: { type: 'number', min: -180, max: 180 },
        gym_radius_meters: { type: 'integer', min: 1, max: 100000 },
        allow_expired_checkin: { type: 'boolean' },
    },
});
const staffCheckinBody = strictBody([
    'member_id',
    'method',
    'notes',
    'latitude',
    ...
    fieldRules: {
        ...
        method: { type: 'string', enum: ['STAFF', 'QR', 'SELF', 'RFID'], caseInsensitive: true },
...
const staffQrCheckinBody = strictBody(['token', 'notes', 'allow_override', 'branch_id'], {
...
const memberQrCheckinBody = strictBody(['token'], {
    requiredFields: ['token'],
    fieldRules: { token: { type: 'string', minLength: 1, maxLength: 4096 } },
});
const memberSelfCheckinBody = strictBody(['latitude', 'longitude'], {
...
const rfidDeviceCreateBody = strictBody(['reader_name', 'reader_serial', 'reader_location'], {
    requiredFields: ['reader_name', 'reader_serial'],
    fieldRules: {
        reader_name: { type: 'string', minLength: 1, maxLength: 120 },
        reader_serial: { type: 'string', minLength: 1, maxLength: 120 },
        reader_location: { type: 'string', maxLength: 240 },
    },
});
const rfidDeviceUpdateBody = strictBody(['reader_name', 'reader_location', 'status'], {
    fieldRules: {
        reader_name: { type: 'string', minLength: 1, maxLength: 120 },
        reader_location: { type: 'string', maxLength: 240 },
        status: { type: 'string', enum: ['ACTIVE', 'PAUSED', 'DISABLED'], caseInsensitive: true },
    },
});
const emptyAttendanceBody = strictBody([], { allowEmpty: true });
const rfidPairBody = strictBody(['member_id', 'tag_id'], {
    requiredFields: ['member_id', 'tag_id'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        tag_id: { type: 'string', minLength: 1, maxLength: 200 },
    },
});
const rfidUnpairBody = strictBody(['member_id'], {
    requiredFields: ['member_id'],
    fieldRules: { member_id: { type: 'integer', min: 1 } },
});
const rfidEventBody = strictBody(['reader_serial', 'tag_id', 'reader_key', 'scanned_at', 'notes'], {
    requiredFields: ['reader_serial', 'tag_id'],
    fieldRules: {
        reader_serial: { type: 'string', minLength: 1, maxLength: 120 },
        tag_id: { type: 'string', minLength: 1, maxLength: 200 },
        reader_key: { type: 'string', minLength: 1, maxLength: 240 },
        scanned_at: { maxLength: 60 },
        notes: { type: 'string', maxLength: 1000 },
    },
});
...
router.put('/mode', auth, saasMiddleware, requireOwner, attendanceModeBody, async (req, res) => {
    try {
...
// --- 1. CHECK-IN A MEMBER ---
router.post('/checkin', auth, saasMiddleware, requirePermission('attendance:write'), staffCheckinBody, async (req, res) => {
...
router.post('/checkin/qr', auth, saasMiddleware, requirePermission('attendance:write'), staffQrCheckinBody, async (req, res) => {
    try {
...
router.post('/member/checkin/qr', memberAuth, saasMiddleware, memberQrCheckinBody, async (req, res) => {
    try {
        const verification = verifyAttendanceToken(req.body?.token);
        if (!verification.valid) {
            return res.status(400).json({ error: verification.reason || 'Invalid QR token.' });
...
router.post('/member/checkin/self', memberAuth, saasMiddleware, memberSelfCheckinBody, async (req, res) => {
    try {
...
router.post('/rfid/devices', auth, saasMiddleware, requireOwner, rfidDeviceCreateBody, async (req, res) => {
...
router.put('/rfid/devices/:id', auth, saasMiddleware, requireOwner, rfidDeviceUpdateBody, async (req, res) => {
...
router.post('/rfid/devices/:id/rotate-secret', auth, saasMiddleware, requireOwner, emptyAttendanceBody, async (req, res) => {
    const deviceId = Number.parseInt(req.params.id, 10);
    if (!Number.isInteger(deviceId)) {
        return res.status(400).json({ error: 'Valid device id is required.' });
    }
...
router.post('/rfid/pair-member', auth, saasMiddleware, requirePermission('members:write'), rfidPairBody, async (req, res) => {
...
router.post('/rfid/unpair-member', auth, saasMiddleware, requirePermission('members:write'), rfidUnpairBody, async (req, res) => {
    const member_id = Number.parseInt(req.body?.member_id, 10);
...
router.post('/rfid/event', rfidEventBody, async (req, res) => {
    ...
    try {
        ...
        if (!device || device.status !== 'ACTIVE' || !safeCompareSecret(device.shared_secret, readerKey)) {
            void recordSecurityEvent(req, {
                ...
                message: 'RFID reader authentication failed.',
                statusCode: 403,
                metadata: { provider: 'rfid', reason: !device ? 'unknown_reader' : 'invalid_reader_credentials' },
            });
