...
const reminderBody = strictBody(['member_ids', 'member_id', 'template_key'], {
    fieldRules: {
...
const campaignBody = strictBody([
    ...
    fieldRules: {
        ...
        channel: { type: 'string', enum: ['WHATSAPP'], caseInsensitive: true },
        message: { type: 'string', maxLength: 4000 },
        dashboard_action_key: { type: 'string', maxLength: 80 },
        dashboard_audience_hash: { type: 'string', maxLength: 120 },
        dashboard_expected_count: { type: 'integer', min: 0, max: 1000000 },
    },
});
const emptyNotificationBody = strictBody([], { allowEmpty: true });
...
router.put('/:id/read', auth, saasMiddleware, emptyNotificationBody, async (req, res) => {
    try {
        await pool.query(
...
router.put('/read-all', auth, saasMiddleware, emptyNotificationBody, async (req, res) => {
    try {
        await pool.query(
...
router.post('/reminders/preview', auth, saasMiddleware, reminderBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
...
router.post('/reminders/send', auth, saasMiddleware, reminderRequestLimiter, reminderBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
...
router.post('/campaign/run', auth, saasMiddleware, requireCampaignAutomationPermission, campaignBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
