...
const savedReportBody = strictBody(['name', 'report_type', 'filters', 'schedule'], {
    requiredFields: ['name'],
    fieldRules: {
        name: { type: 'string', minLength: 1, maxLength: 120 },
        report_type: { type: 'string', maxLength: 60 },
        filters: { type: 'object' },
        schedule: { type: 'string', maxLength: 120 },
    },
});
const emptyExportBody = strictBody([], { allowEmpty: true });
...
router.post('/saved-reports', requirePermission('insights:read'), savedReportBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.delete('/saved-reports/:id', requirePermission('insights:read'), emptyExportBody, async (req, res) => {
    try {
        const gid = gymId(req);
