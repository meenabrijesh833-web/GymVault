...
const { getRuntimeTelemetrySnapshot, listRuntimeEvents, recordSecurityEvent } = require('../utils/runtimeTelemetry');
...
const superadminLoginBody = strictBody(['password'], {
    requiredFields: ['password'],
    fieldRules: { password: { type: 'string', minLength: 1, maxLength: 256 } },
});
const superadminGymBody = strictBody([
    'gym_name',
    'phone',
    'address',
    'support_email',
    ...
    fieldRules: {
        gym_name: { type: 'string', minLength: 2, maxLength: 120 },
        phone: { type: 'string', format: 'phone', maxLength: 30 },
        address: { type: 'string', maxLength: 500 },
        support_email: { type: 'string', format: 'email', maxLength: 120 },
        website: { type: 'string', maxLength: 2048 },
        plan: { type: 'string', enum: ['test', 'basic', 'growth', 'pro'] },
        addon_extra_whatsapp: { type: 'integer', min: 0, max: 1000000 },
        addon_extra_staff: { type: 'integer', min: 0, max: 1000000 },
        addon_extra_members: { type: 'integer', min: 0, max: 1000000 },
        addon_extra_branches: { type: 'integer', min: 0, max: 1000000 },
        addon_extra_hello: { type: 'integer', min: 0, max: 1000000 },
    },
});
const superadminGymStatusBody = strictBody(['status', 'reason'], {
...
const superadminPasswordResetBody = strictBody(['new_password'], {
    requiredFields: ['new_password'],
    fieldRules: { new_password: { type: 'string', minLength: 8, maxLength: 256 } },
});
const superadminUserBlockBody = strictBody(['blocked'], {
    requiredFields: ['blocked'],
    fieldRules: { blocked: { type: 'boolean' } },
});
const superadminTicketReplyBody = strictBody(['message'], {
    requiredFields: ['message'],
    fieldRules: { message: { type: 'string', minLength: 1, maxLength: 5000 } },
});
const superadminTicketUpdateBody = strictBody(['status', 'priority', 'tags'], {
    allowEmpty: true,
    fieldRules: {
...
const superadminSystemBody = strictBody([
    'maintenance_mode',
    'maintenance_message',
    'feature_flags',
    'support_profile',
    ...
    fieldRules: {
        ...
        maintenance_message: { type: 'string', maxLength: 1000 },
        feature_flags: { type: 'object' },
        support_profile: { type: 'object' },
        automation_settings: { type: 'object' },
        billing_config: { type: 'object' },
    },
});
const superadminBroadcastBody = strictBody(['title', 'message', 'url', 'roles', 'target_gym_id'], {
    requiredFields: ['title', 'message'],
    fieldRules: {
        title: { type: 'string', minLength: 1, maxLength: 160 },
        message: { type: 'string', minLength: 1, maxLength: 4000 },
        url: { type: 'string', pattern: /^(?:\/(?!\/)|https?:\/\/)/i, maxLength: 2048 },
        roles: {
            type: 'array',
            minItems: 1,
            maxItems: 3,
            items: { type: 'string', enum: ['OWNER', 'STAFF', 'MEMBER'], caseInsensitive: true },
        },
        target_gym_id: { type: 'integer', min: 1 },
    },
});
const emptySuperadminBody = strictBody([], { allowEmpty: true });
...
const superadminLoginLimiter = rateLimit({
    ...
    handler: (req, res) => {
        void recordSecurityEvent(req, {
            ...
            message: 'Superadmin login rate limit was reached.',
            statusCode: 429,
            metadata: { flow: 'superadmin', reason: 'rate_limit' },
        });
...
const superAuth = (req, res, next) => {
    ...
    if (!isIpAllowed(req)) {
        void recordSecurityEvent(req, {
            eventType: 'AUTH_FAILED',
            message: 'Superadmin request was denied by the IP allowlist.',
            statusCode: 403,
            metadata: { flow: 'superadmin', reason: 'ip_not_allowed' },
        });
    ...
    if (!token) {
        void recordSecurityEvent(req, {
            ...
            message: 'Superadmin request was missing authentication.',
            statusCode: 401,
            metadata: { flow: 'superadmin' },
        });
        return res.status(401).json({ message: 'Master access denied' });
    }
    ...
    try {
        ...
        void recordSecurityEvent(req, {
            ...
            message: 'Superadmin token verification failed.',
            statusCode: 401,
            metadata: { flow: 'superadmin' },
        });
...
router.post('/login', superadminLoginLimiter, superadminLoginBody, (req, res) => {
    if (!superadminEnabled) {
        return res.status(503).json({ message: disabledMessage });
    }
    ...
    if (!isIpAllowed(req)) {
        void recordSecurityEvent(req, {
            eventType: 'AUTH_FAILED',
            message: 'Superadmin login was denied by the IP allowlist.',
            statusCode: 403,
            metadata: { flow: 'superadmin', reason: 'ip_not_allowed' },
        });
        return res.status(403).json({ message: 'Access denied from this IP address.' });
    ...
    void recordSecurityEvent(req, {
        eventType: 'AUTH_FAILED',
        message: 'Superadmin password authentication failed.',
        statusCode: 401,
        metadata: { flow: 'superadmin', reason: 'invalid_password' },
    });
...
router.put('/gyms/:id', superAuth, superadminGymBody, async (req, res) => {
    const gymId = parseInt(req.params.id, 10);
    if (!Number.isInteger(gymId)) return res.status(400).json({ error: 'Invalid gym id' });
...
router.put('/gyms/:id/status', superAuth, superadminGymStatusBody, async (req, res) => {
    const gymId = parseInt(req.params.id, 10);
...
router.delete('/gyms/:id', superAuth, emptySuperadminBody, async (req, res) => {
    const gymId = parseInt(req.params.id, 10);
    if (!Number.isInteger(gymId)) return res.status(400).json({ error: 'Invalid gym id' });
...
router.post('/gyms/:id/impersonate', superAuth, emptySuperadminBody, async (req, res) => {
    const gymId = parseInt(req.params.id, 10);
    if (!Number.isInteger(gymId)) return res.status(400).json({ error: 'Invalid gym id' });
...
router.post('/users/:id/reset-password', superAuth, superadminPasswordResetBody, async (req, res) => {
...
router.put('/users/:id/block', superAuth, superadminUserBlockBody, async (req, res) => {
    const userId = parseInt(req.params.id, 10);
...
router.delete('/users/:id', superAuth, emptySuperadminBody, async (req, res) => {
    const userId = parseInt(req.params.id, 10);
    if (!Number.isInteger(userId)) return res.status(400).json({ error: 'Invalid user id' });
...
router.post('/support/tickets/:id/reply', superAuth, superadminTicketReplyBody, async (req, res) => {
    const ticketId = parseInt(req.params.id, 10);
...
router.put('/support/tickets/:id', superAuth, superadminTicketUpdateBody, async (req, res) => {
    const ticketId = parseInt(req.params.id, 10);
    if (!Number.isInteger(ticketId)) return res.status(400).json({ error: 'Invalid ticket id' });
...
router.put('/system', superAuth, superadminSystemBody, async (req, res) => {
...
router.post('/system/broadcast', superAuth, superadminBroadcastBody, async (req, res) => {
