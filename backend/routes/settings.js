...
const customTemplateBody = strictBody(['raw_text', 'title'], {
    requiredFields: ['raw_text'],
    fieldRules: {
        raw_text: { type: 'string', minLength: 5, maxLength: 4000 },
        title: { type: 'string', maxLength: 120 },
    },
});
const integrationsBody = strictBody([
    'save_scope',
    'owner_mobile',
    'whatsapp_number',
    'bulk_enabled',
    ...
    fieldRules: {
        save_scope: { type: 'string', enum: ['payments', 'messaging', 'campaigns', 'all'], caseInsensitive: true },
        owner_mobile: { type: 'string', format: 'phone', maxLength: 30 },
        whatsapp_number: { type: 'string', format: 'phone', maxLength: 30 },
        bulk_enabled: { type: 'boolean' },
        bulk_per_campaign_limit: { type: 'integer', min: 1, max: 1000 },
        bulk_channels: { type: 'object' },
        templates: { type: 'array', maxItems: 50 },
        member_payments: { type: 'object' },
    },
});
const integrationTestBody = strictBody(['to', 'template_key'], {
    requiredFields: ['to', 'template_key'],
    fieldRules: {
        to: { type: 'string', format: 'phone', maxLength: 40 },
        template_key: { type: 'string', minLength: 1, maxLength: 60 },
    },
});
const branchSettingsBody = strictBody(['city', 'branches_count', 'branch_directory'], {
    requiredFields: ['branches_count'],
    fieldRules: {
        city: { type: 'string', maxLength: 100 },
        branches_count: { type: 'integer', min: 1, max: 25 },
        branch_directory: { type: 'array', minItems: 1, maxItems: 25 },
    },
});
const apiKeyBody = strictBody(['key_name', 'scopes'], {
    requiredFields: ['key_name', 'scopes'],
    fieldRules: {
        key_name: { type: 'string', minLength: 2, maxLength: 120 },
        scopes: {
            type: 'array',
            minItems: 1,
            maxItems: 7,
            items: {
                type: 'string',
                enum: ['members:read', 'members:write', 'payments:read', 'payments:write', 'attendance:read', 'attendance:write', 'dashboard:read'],
            },
        },
    },
});
const webhookBody = strictBody(['url', 'events', 'secret', 'is_active'], {
    ...
    fieldRules: {
        ...
        events: {
            ...
            items: {
                type: 'string',
                enum: ['member.created', 'member.updated', 'payment.recorded', 'attendance.checked_in', 'class.booking.created'],
            },
        },
        secret: { type: 'string', maxLength: 240 },
        is_active: { type: 'boolean' },
    },
});
const memberImportBody = strictBody(['csv_text', 'dry_run'], {
    requiredFields: ['csv_text'],
    fieldRules: {
        csv_text: { type: 'string', minLength: 1, maxLength: 500000 },
        dry_run: { type: 'boolean' },
    },
});
const accountSettingsBody = strictBody([
    'full_name',
    'email',
    'phone',
    'current_password',
    ...
    fieldRules: {
        ...
        current_password: { type: 'string', maxLength: 256 },
        new_password: { type: 'string', minLength: 8, maxLength: 256 },
        remove_profile_pic: { type: 'boolean' },
    },
});
const gymSettingsBody = strictBody([
    'name',
    'phone',
    'email',
    'address',
    ...
    fieldRules: {
        ...
        support_whatsapp: { type: 'string', format: 'phone', maxLength: 30 },
        support_window: { type: 'string', maxLength: 500 },
        support_sla: { type: 'string', maxLength: 500 },
        support_about_mission: { type: 'string', maxLength: 4000 },
        gym_logo: { type: 'string', maxLength: 800000 },
        owner_signature: { type: 'string', maxLength: 800000 },
    },
});
const preferenceSettingsBody = strictBody([
    'currency',
    'timezone',
    ...
    requiredFields: ['currency', 'timezone'],
    fieldRules: {
        currency: { type: 'string', minLength: 1, maxLength: 10 },
        timezone: { type: 'string', minLength: 3, maxLength: 100 },
        interface_reduce_motion: { type: 'boolean' },
        interface_compact_mode: { type: 'boolean' },
        interface_dark_mode: { type: 'boolean' },
    },
});
const gymDeletionBody = strictBody(['current_password'], {
    requiredFields: ['current_password'],
    fieldRules: { current_password: { type: 'string', minLength: 1, maxLength: 256 } },
});
const emptySettingsBody = strictBody([], { allowEmpty: true });
    ...
    try {
        ...
        if (expectedToken && suppliedToken !== expectedToken) {
            void recordSecurityEvent(req, {
                ...
                message: 'MSG91 delivery webhook authentication failed.',
                statusCode: 401,
                metadata: { provider: 'msg91', reason: suppliedToken ? 'invalid_token' : 'missing_token' },
            });
...
const ensureSupportProfileTable = async () => {
    if (!ensureSupportProfileTablePromise) {
        ensureSupportProfileTablePromise = adminPool.query(`
...
router.post('/integrations/templates/custom', auth, customTemplateBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
...
router.delete('/integrations/templates/:templateKey', auth, emptySettingsBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
...
router.put('/integrations', auth, integrationsBody, async (req, res) => {
    let client;
...
router.post('/integrations/test-message', auth, integrationTestBody, async (req, res) => {
    try {
        await ensureMessagingSchema();
...
router.put('/platform/branches', auth, branchSettingsBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
        await ensureBillingAddonSchema();
...
router.post('/platform/api-keys', auth, apiKeyBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
...
router.delete('/platform/api-keys/:id', auth, emptySettingsBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
...
router.post('/platform/webhooks', auth, webhookBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
...
router.put('/platform/webhooks/:id', auth, webhookBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
...
router.delete('/platform/webhooks/:id', auth, emptySettingsBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
        const webhookId = ensureInteger(req.params.id, { field: 'webhook id', required: true, min: 1 });
        const result = await pool.query(
...
router.post('/platform/webhooks/:id/test', auth, emptySettingsBody, async (req, res) => {
    try {
        await ensurePlatformSchema();
        const webhookId = ensureInteger(req.params.id, { field: 'webhook id', required: true, min: 1 });
...
router.post('/import/members', auth, memberImportBody, async (req, res) => {
    let client;
    try {
...
router.put('/account', auth, uploadProfilePic, accountSettingsBody, async (req, res) => {
...
router.put('/gym', auth, gymSettingsBody, async (req, res) => {
    try {
        await ensureSupportProfileTable();
        ...
        const gym_logo = hasGymLogo
            ? (rawGymLogo ? await sanitizeInlineImageDataUrl(rawGymLogo, {
                req,
                field: 'gym_logo',
                maxBytes: 512 * 1024,
                maxPixels: 4_000_000,
        ...
        const owner_signature = hasOwnerSignature
            ? (rawOwnerSignature ? await sanitizeInlineImageDataUrl(rawOwnerSignature, {
                req,
...
router.put('/preferences', auth, preferenceSettingsBody, async (req, res) => {
    try {
        await ensurePreferenceSchema();
...
// --- 6. DANGER ZONE ---
router.delete('/nuke', auth, gymDeletionBody, async (req, res) => {
