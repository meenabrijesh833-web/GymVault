...
const leadBody = strictBody([
    ...
    fieldRules: {
        ...
        status: { type: 'string', enum: ['NEW', 'CONTACTED', 'FOLLOW_UP', 'TRIAL_BOOKED', 'WON', 'LOST'], caseInsensitive: true },
        priority: { type: 'string', enum: ['HIGH', 'MEDIUM', 'LOW'], caseInsensitive: true },
        notes: { type: 'string', maxLength: 2000 },
        lost_reason: { type: 'string', maxLength: 500 },
        next_follow_up_at: { maxLength: 60 },
        trial_date: { maxLength: 60 },
        mark_contacted: { type: 'boolean' },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const leadChatBody = strictBody(['template_key', 'message'], {
    fieldRules: {
        template_key: { type: 'string', maxLength: 60 },
        message: { type: 'string', maxLength: 700 },
    },
});
const emptyLeadBody = strictBody([], { allowEmpty: true });
...
router.post('/:id/chat/messages', requirePermission('members:write'), leadChatBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
        const leadId = ensureInteger(req.params.id, { field: 'lead id', required: true, min: 1 });
        const lead = await loadLeadById(gymId, leadId);
        ...
        return res.status(500).json({ error: 'Failed to send WhatsApp reply.' });
    }
});

router.post('/', requirePermission('members:write'), leadBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.put('/:id', requirePermission('members:write'), leadBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
        const leadId = ensureInteger(req.params.id, { field: 'lead id', required: true, min: 1 });
        const payload = normalizeLeadPayload(req.body || {});
...
router.post('/:id/convert', requirePermission('members:write'), emptyLeadBody, async (req, res) => {
    const gymId = getGymIdFromRequest(req);
    let leadId;
...
router.delete('/:id', requirePermission('members:write'), emptyLeadBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
        const leadId = ensureInteger(req.params.id, { field: 'lead id', required: true, min: 1 });
