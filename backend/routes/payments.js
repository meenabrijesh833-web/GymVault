...
const paymentRecordBody = strictBody([
    'user_id',
    'plan_id',
    'amount_paid',
    'total_amount',
...
const duePaymentLinkStatusBody = strictBody(['payment_link_id', 'amount', 'notes', 'branch_id'], {
...
const dueCollectionBody = strictBody(['amount', 'payment_mode', 'transaction_id', 'notes', 'branch_id'], {
    fieldRules: {
        amount: { type: 'number', min: 0, max: 1000000 },
        payment_mode: { type: 'string', enum: ['CASH', 'ONLINE'], caseInsensitive: true },
        transaction_id: { type: 'string', maxLength: 120 },
...
const dueVerificationBody = strictBody([
    'amount',
    'notes',
    'razorpay_order_id',
    'razorpay_payment_id',
...
const ensurePaymentCollectionsSchema = async () => {
    if (!ensurePaymentCollectionsSchemaPromise) {
        ensurePaymentCollectionsSchemaPromise = adminPool.query(`
                ALTER TABLE gyms
    ...
    try {
        ...
        await ensurePartnerTransferForCapturedPayment({
            ...
            notes: {
                ...
                gym_id: String(gym_id),
                payment_id: String(paymentId),
                member_id: String(paymentLink?.notes?.member_id || ''),
...
router.post('/:id/due/verify', dueVerificationBody, async (req, res) => {
    try {
        ...
        if (!paymentSignatureMatches(expectedSign, razorpay_signature)) {
            void recordPaymentSignatureRejection(req, 'member_due');
