const { createValidatedRouter } = require('../middleware/validatedRouter');
...
const expenseBody = strictBody([
    ...
    fieldRules: {
        ...
        bill_date: { type: 'string', maxLength: 60 },
        payment_mode: { type: 'string', maxLength: 40 },
        is_recurring: { type: 'boolean' },
        recurrence_rule: { type: 'string', maxLength: 120 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const payrollPayoutSettingsBody = strictBody([
    'default_online_channel',
    'payout_note_prefix',
    'allow_cash_payouts',
    ...
    allowEmpty: true,
    fieldRules: {
        default_online_channel: { type: 'string', enum: ['UPI', 'BANK_TRANSFER'], caseInsensitive: true },
        payout_note_prefix: { type: 'string', maxLength: 120 },
        allow_cash_payouts: { type: 'boolean' },
        allow_manual_bank_transfer: { type: 'boolean' },
    },
});
const payrollStaffDestinationBody = strictBody([
    'user_id',
    'upi_id',
    'bank_account_holder',
    'bank_account_number',
    ...
    fieldRules: {
        ...
        upi_id: { type: 'string', pattern: /^[a-z0-9._-]{2,}@[a-z][a-z0-9.-]{1,}$/i, maxLength: 120 },
        bank_account_holder: { type: 'string', maxLength: 120 },
        bank_account_number: { type: 'string', pattern: /^\d(?:[\d ]{4,22}\d)$/, maxLength: 30 },
        bank_ifsc: { type: 'string', pattern: /^[A-Z]{4}0[A-Z0-9]{6}$/i },
        bank_name: { type: 'string', maxLength: 120 },
        notes: { type: 'string', maxLength: 500 },
        clear_bank_account: { type: 'boolean' },
        remove_destination: { type: 'boolean' },
    },
});
const payrollCreateBody = strictBody([
    'user_id',
    'pay_period',
    'base_pay',
    'commission',
    ...
    requiredFields: ['user_id', 'pay_period', 'base_pay'],
    fieldRules: {
        user_id: { type: 'integer', min: 1 },
        pay_period: { type: 'string', minLength: 1, maxLength: 30 },
        base_pay: { type: 'number', min: 0, max: 10000000 },
        commission: { type: 'number', min: 0, max: 10000000 },
        deductions: { type: 'number', min: 0, max: 10000000 },
        notes: { type: 'string', maxLength: 1000 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const payrollUpdateBody = strictBody([
    'base_pay',
    'commission',
    'deductions',
    'notes',
    ...
    fieldRules: {
        ...
        deductions: { type: 'number', min: 0, max: 10000000 },
        notes: { type: 'string', maxLength: 1000 },
        status: { type: 'string', enum: ['PENDING_APPROVAL', 'APPROVED', 'PAID', 'REJECTED'], caseInsensitive: true },
        rejection_reason: { type: 'string', maxLength: 500 },
        payout_mode: { type: 'string', enum: ['CASH', 'ONLINE'], caseInsensitive: true },
        payout_channel: { type: 'string', enum: ['CASH', 'UPI_INTENT', 'BANK_TRANSFER'], caseInsensitive: true },
        payout_destination_label: { type: 'string', maxLength: 160 },
        payout_reference: { type: 'string', maxLength: 120 },
        payout_notes: { type: 'string', maxLength: 1000 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const payrollAutoConfigBody = strictBody(['user_id', 'base_pay', 'pay_day', 'auto_enabled'], {
    fieldRules: {
        user_id: { type: 'integer', min: 1 },
        base_pay: { type: 'number', min: 0, max: 10000000 },
        pay_day: { type: 'integer', min: 1, max: 28 },
        auto_enabled: { type: 'boolean' },
    },
});
const posProductBody = strictBody([
    'name',
    'price',
    'category',
    'cost_price',
    ...
    fieldRules: {
        ...
        category: { type: 'string', maxLength: 60 },
        cost_price: { type: 'number', min: 0, max: 1000000 },
        stock_qty: { type: 'number', min: 0, max: 1000000 },
        low_stock_threshold: { type: 'number', min: 0, max: 1000000 },
        sku: { type: 'string', maxLength: 120 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const posProductUpdateBody = strictBody([
    'name',
    'price',
    'category',
    'cost_price',
...
const posSaleBody = strictBody(['member_id', 'items', 'payment_mode', 'notes', 'branch_id'], {
    requiredFields: ['items'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        items: { type: 'array', minItems: 1, maxItems: 100 },
        payment_mode: { type: 'string', enum: ['CASH', 'ONLINE', 'CARD', 'UPI'], caseInsensitive: true },
        notes: { type: 'string', maxLength: 1000 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const posVoidBody = strictBody(['reason'], {
    allowEmpty: true,
    fieldRules: { reason: { type: 'string', maxLength: 500 } },
});
const posStockAdjustmentBody = strictBody(['adjustment', 'reason'], {
    requiredFields: ['adjustment'],
    fieldRules: {
        adjustment: { type: 'integer', min: -1000000, max: 1000000 },
        reason: { type: 'string', maxLength: 500 },
    },
});
const accessPolicyBody = strictBody([
    'plan_id',
    'name',
    'allowed_days',
    'allowed_from',
    ...
    allowEmpty: true,
    fieldRules: {
        plan_id: { type: 'integer', min: 1 },
        name: { type: 'string', maxLength: 120 },
        allowed_days: { type: 'string', pattern: /^(?:MON|TUE|WED|THU|FRI|SAT|SUN)(?:,(?:MON|TUE|WED|THU|FRI|SAT|SUN))*$/i, maxLength: 27 },
        allowed_from: { type: 'string', pattern: /^\d{2}:\d{2}(?::\d{2})?$/ },
        allowed_to: { type: 'string', pattern: /^\d{2}:\d{2}(?::\d{2})?$/ },
        is_offpeak_only: { type: 'boolean' },
        enforce_freeze: { type: 'boolean' },
        max_daily_visits: { type: 'integer', min: 0, max: 1000 },
    },
});
const accessPolicyUpdateBody = strictBody([
    'plan_id',
    'name',
    'allowed_days',
    'allowed_from',
...
const emptyFinanceBody = strictBody([], { allowEmpty: true });
const optionalBranchBody = strictBody(['branch_id'], {
    allowEmpty: true,
    fieldRules: { branch_id: { type: 'string', maxLength: 60 } },
});
...
router.post('/expenses', requirePermission('payments:write'), expenseBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.put('/expenses/:id', requirePermission('payments:write'), expenseBody, async (req, res) => {
    try {
...
router.delete('/expenses/:id', requirePermission('payments:write'), optionalBranchBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
...
router.put('/payroll/payout-settings', requirePermission('payments:write'), payrollPayoutSettingsBody, async (req, res) => {
    try {
        if (!isOwnerUser(req)) {
...
router.put('/payroll/staff-destinations/:userId', requirePermission('payments:write'), payrollStaffDestinationBody, async (req, res) => {
    try {
        if (!isOwnerUser(req)) {
...
router.delete('/payroll/staff-destinations/:userId', requirePermission('payments:write'), emptyFinanceBody, async (req, res) => {
    try {
        if (!isOwnerUser(req)) {
...
router.post('/payroll', requirePermission('payments:write'), payrollCreateBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.put('/payroll/:id', requirePermission('payments:write'), payrollUpdateBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
        const existingResult = await pool.query(
...
router.delete('/payroll/:id', requirePermission('payments:write'), emptyFinanceBody, async (req, res) => {
    try {
        if (!isOwnerUser(req)) {
...
router.put('/payroll/auto-config/:userId', requirePermission('payments:write'), payrollAutoConfigBody, async (req, res) => {
    try {
        const gid = req.user.gym_id;
...
router.post('/pos/products', requirePermission('payments:write'), ensurePosPlanAccess, posProductBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.put('/pos/products/:id', requirePermission('payments:write'), ensurePosPlanAccess, posProductUpdateBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
...
router.delete('/pos/products/:id', requirePermission('payments:write'), ensurePosPlanAccess, emptyFinanceBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
...
router.post('/pos/sales', requirePermission('payments:write'), ensurePosPlanAccess, posSaleBody, async (req, res) => {
...
// ── POS: Void Sale ──
router.post('/pos/sales/:id/void', requirePermission('payments:write'), ensurePosPlanAccess, posVoidBody, async (req, res) => {
    const client = await pool.connect();
    try {
        const gid = gymId(req);
...
router.post('/pos/products/:id/adjust-stock', requirePermission('payments:write'), ensurePosPlanAccess, posStockAdjustmentBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.post('/access-policies', requirePermission('attendance:write'), accessPolicyBody, async (req, res) => {
    try {
        const gid = gymId(req);
...
router.put('/access-policies/:id', requirePermission('attendance:write'), accessPolicyUpdateBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
...
router.delete('/access-policies/:id', requirePermission('attendance:write'), emptyFinanceBody, async (req, res) => {
    try {
        const gid = gymId(req); const id = posInt(req.params.id);
        if (!id) return res.status(400).json({ error: 'Invalid id.' });
