...
const billingCheckoutBody = strictBody(['plan_tier', 'cycle', 'coupon_code', 'plan_setup'], {
    requiredFields: ['plan_tier', 'cycle'],
    fieldRules: {
        plan_tier: { type: 'string', enum: BILLING_PLAN_ORDER, caseInsensitive: true },
        cycle: { type: 'string', enum: ['monthly', 'semiannual', 'annual'], caseInsensitive: true },
        coupon_code: { type: 'string', maxLength: 32, pattern: /^[A-Za-z0-9_-]+$/ },
        plan_setup: { type: 'object' },
    },
});
const billingVerificationBody = strictBody([
    'razorpay_order_id',
    'razorpay_payment_id',
    'razorpay_signature',
    'plan_tier',
    ...
    requiredFields: ['razorpay_order_id', 'razorpay_payment_id', 'razorpay_signature', 'plan_tier', 'cycle'],
    fieldRules: {
...
const addonOrderBody = strictBody(['addon_key'], {
    requiredFields: ['addon_key'],
    fieldRules: {
        addon_key: { type: 'string', enum: BILLING_ADDON_ORDER },
    },
});
const addonVerificationBody = strictBody([
    'razorpay_order_id',
    'razorpay_payment_id',
    'razorpay_signature',
    'addon_key',
], {
    requiredFields: ['razorpay_order_id', 'razorpay_payment_id', 'razorpay_signature', 'addon_key'],
    fieldRules: {
        razorpay_order_id: { type: 'string', minLength: 1, maxLength: 120 },
        razorpay_payment_id: { type: 'string', minLength: 1, maxLength: 120 },
        razorpay_signature: { type: 'string', minLength: 1, maxLength: 256 },
...
const emptyBillingBody = strictBody([], { allowEmpty: true });
...
const ensureBillingCouponRedemptionSchema = async () => {
    if (!ensureBillingCouponRedemptionSchemaPromise) {
        ensureBillingCouponRedemptionSchemaPromise = adminPool.query(`
...
router.post('/preview', billingCheckoutBody, async (req, res) => {
    try {
...
router.post('/create-order', billingCheckoutBody, async (req, res) => {
    try {
        const billingConfig = await getBillingConfig();
        const resolved = resolveSaasPrice(billingConfig, req.body.plan_tier, req.body.cycle);
        if (!resolved) {
...
// --- 2. VERIFY & SAVE PLAN ---
router.post('/verify', billingVerificationBody, async (req, res) => {
    ...
    if (paymentSignatureMatches(expectedSign, razorpay_signature)) {
        ...
        void recordPaymentSignatureRejection(req, 'saas_subscription');
        res.status(400).json({ error: "Invalid signature!" });
    }
});

// --- 3. RESET TEST PLAN TIMER (dev only, only works if current_plan is 'test') ---
router.post('/reset-test', emptyBillingBody, async (req, res) => {
...
router.post('/create-addon-order', addonOrderBody, async (req, res) => {
    try {
...
router.post('/verify-addon', addonVerificationBody, async (req, res) => {
    ...
    if (!paymentSignatureMatches(expectedSign, razorpay_signature)) {
        void recordPaymentSignatureRejection(req, 'saas_addon');
