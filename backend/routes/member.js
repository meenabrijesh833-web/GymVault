...
const memberDueOrderBody = strictBody(['amount'], {
    allowEmpty: true,
    fieldRules: { amount: { type: 'number', min: 0, max: 1000000 } },
});
const memberDuePaymentLinkStatusBody = strictBody(['payment_link_id', 'amount', 'notes'], {
    requiredFields: ['payment_link_id'],
    fieldRules: {
...
const memberRenewalOrderBody = strictBody(['plan_id'], {
    allowEmpty: true,
    fieldRules: { plan_id: { type: 'integer', min: 1 } },
});
const memberRenewalPaymentLinkStatusBody = strictBody(['payment_link_id', 'plan_id'], {
    requiredFields: ['payment_link_id'],
    fieldRules: {
        payment_link_id: { type: 'string', minLength: 1, maxLength: 120 },
...
const memberProfileBody = strictBody(['full_name', 'email', 'phone', 'remove_profile_pic'], {
...
const memberClassBookingBody = strictBody(['session_id', 'notes'], {
...
router.put('/profile', uploadMemberProfilePic, memberProfileBody, async (req, res) => {
    try {
...
router.patch('/onboarding', memberOnboardingBody, async (req, res) => {
    try {
...
router.post('/documents', memberDocumentBody, async (req, res) => {
    try {
...
router.delete('/documents/:id', emptyMemberBody, async (req, res) => {
    try {
...
router.post('/payments/dues/:paymentId/create-order', memberDueOrderBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
...
router.post('/payments/dues/:paymentId/payment-link-status', memberDuePaymentLinkStatusBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
        const paymentId = ensureInteger(req.params.paymentId, { field: 'payment id', required: true, min: 1 });
...
router.post('/membership/renew/create-order', memberRenewalOrderBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
...
router.post('/membership/renew/payment-link-status', memberRenewalPaymentLinkStatusBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
        const paymentLinkId = ensureTrimmedString(req.body?.payment_link_id, { field: 'payment_link_id', required: true, max: 120 });
...
router.post('/classes/bookings', memberClassBookingBody, async (req, res) => {
    try {
...
router.delete('/classes/bookings/:bookingId', emptyMemberBody, async (req, res) => {
    try {
