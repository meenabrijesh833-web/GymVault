...
const { sanitizeInlineImageList } = require('../utils/inlineImageSafety');
...
const STAFF_ROLES = ['MANAGER', 'RECEPTION', 'TRAINER', 'WORKER', 'CLEANER', 'ACCOUNTANT', 'STAFF'];
const staffCreateBody = strictBody(['full_name', 'email', 'password', 'staff_role', 'permissions', 'branch_id'], {
    requiredFields: ['full_name', 'email', 'password'],
    fieldRules: {
        full_name: { type: 'string', minLength: 1, maxLength: 120 },
        email: { type: 'string', format: 'email', maxLength: 254 },
        password: { type: 'string', minLength: 8, maxLength: 256 },
        staff_role: { type: 'string', enum: STAFF_ROLES, caseInsensitive: true },
        permissions: { type: 'array', maxItems: 100 },
        branch_id: { type: 'string', maxLength: 60 },
    },
});
const staffUpdateBody = strictBody(['full_name', 'staff_role', 'is_active', 'permissions', 'branch_id'], {
    fieldRules: {
        full_name: { type: 'string', minLength: 1, maxLength: 120 },
        staff_role: { type: 'string', enum: STAFF_ROLES, caseInsensitive: true },
...
const staffPasswordResetBody = strictBody(['new_password'], {
    requiredFields: ['new_password'],
    fieldRules: { new_password: { type: 'string', minLength: 8, maxLength: 256 } },
});
const staffTaskBody = strictBody(['assigned_to', 'title', 'description', 'category', 'priority', 'due_at'], {
    requiredFields: ['assigned_to', 'title', 'category', 'priority', 'due_at'],
    fieldRules: {
        assigned_to: { type: 'integer', min: 1 },
        title: { type: 'string', minLength: 3, maxLength: 160 },
        description: { type: 'string', maxLength: 2000 },
        category: { type: 'string', enum: STAFF_TASK_CATEGORIES, caseInsensitive: true },
        priority: { type: 'string', enum: STAFF_TASK_PRIORITIES, caseInsensitive: true },
        due_at: { maxLength: 60 },
    },
});
const staffTaskStatusBody = strictBody(['status'], {
    requiredFields: ['status'],
    fieldRules: { status: { type: 'string', enum: ['OPEN', 'IN_PROGRESS', 'CANCELLED'], caseInsensitive: true } },
});
const staffTaskCompletionBody = strictBody(['completion_notes', 'completion_photos'], {
    allowEmpty: true,
    fieldRules: {
        completion_notes: { type: 'string', maxLength: 2000 },
        completion_photos: { type: 'array', maxItems: 4 },
    },
});
const emptyUserBody = strictBody([], { allowEmpty: true });
...
const ensureStaffTaskSchema = async () => {
    if (!ensureStaffTaskSchemaPromise) {
        ensureStaffTaskSchemaPromise = adminPool.query(`
...
const normalizeTaskCompletionPayload = async (req, payload = {}) => ({
    completion_notes: ensureTrimmedString(payload.completion_notes, { field: 'completion_notes', max: 2000 }),
    completion_photos: await sanitizeInlineImageList(payload.completion_photos, {
        req,
...
router.post('/staff', auth, requireOwner, saasMiddleware, staffCreateBody, async (req, res) => {
...
router.put('/staff/:id', auth, requireOwner, saasMiddleware, staffUpdateBody, async (req, res) => {
...
router.delete('/staff/:id', auth, requireOwner, saasMiddleware, emptyUserBody, async (req, res) => {
    const staffId = parseInt(req.params.id, 10);
...
// POST /api/users/staff/:id/reset-password — Owner resets staff password
router.post('/staff/:id/reset-password', auth, requireOwner, saasMiddleware, staffPasswordResetBody, async (req, res) => {
    const staffId = parseInt(req.params.id, 10);
...
router.post('/tasks', auth, requireOwner, saasMiddleware, staffTaskBody, async (req, res) => {
    try {
        await ensureStaffTaskSchema();
...
router.patch('/tasks/:id/status', auth, saasMiddleware, staffTaskStatusBody, async (req, res) => {
    try {
        await ensureStaffTaskSchema();
...
router.post('/tasks/:id/complete', auth, saasMiddleware, staffTaskCompletionBody, async (req, res) => {
    try {
        await ensureStaffTaskSchema();
        const taskId = ensureInteger(req.params.id, { field: 'task id', required: true, min: 1 });
        const currentTask = await getStaffTaskRowById(taskId, req.user.gym_id);
        ...
        const payload = await normalizeTaskCompletionPayload(req, req.body || {});
        await pool.query(
            `UPDATE staff_tasks
...
router.delete('/tasks/:id', auth, requireOwner, saasMiddleware, emptyUserBody, async (req, res) => {
    try {
        await ensureStaffTaskSchema();
        const taskId = ensureInteger(req.params.id, { field: 'task id', required: true, min: 1 });
