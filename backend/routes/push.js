...
const pushSubscriptionBody = strictBody(['endpoint', 'expirationTime', 'keys'], {
    requiredFields: ['endpoint', 'keys'],
    fieldRules: {
        endpoint: { type: 'string', format: 'url', minLength: 1, maxLength: MAX_PUSH_ENDPOINT_LENGTH },
        expirationTime: { type: 'integer', min: 0 },
        keys: { type: 'object' },
    },
});
const pushUnsubscribeBody = strictBody(['endpoint'], {
    requiredFields: ['endpoint'],
    fieldRules: {
        endpoint: { type: 'string', format: 'url', minLength: 1, maxLength: MAX_PUSH_ENDPOINT_LENGTH },
    },
});
const emptyPushBody = strictBody([], { allowEmpty: true });
if (vapidConfigured) {
...
router.post('/subscribe', authMiddleware, pushSubscriptionBody, async (req, res) => {
...
router.post('/subscribe-member', memberAuthMiddleware, pushSubscriptionBody, async (req, res) => {
    const parsedSubscription = parsePushSubscription(req.body);
    if (parsedSubscription.error) {
...
router.delete('/unsubscribe', authMiddleware, pushUnsubscribeBody, async (req, res) => {
    const endpoint = String(req.body?.endpoint || '').trim();
    if (!endpoint || endpoint.length > MAX_PUSH_ENDPOINT_LENGTH) return res.status(400).json({ message: 'Endpoint required.' });

    try {
        await pool.query(
            'DELETE FROM push_subscriptions WHERE endpoint = $1 AND gym_id = $2 AND user_id = $3',
            [endpoint, req.user.gym_id, req.user.id]
        );
...
router.post('/test', authMiddleware, emptyPushBody, async (req, res) => {
    const gym_id = req.user.gym_id;
    const user_id = req.user.id;
