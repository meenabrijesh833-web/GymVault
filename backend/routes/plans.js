...
const planBody = strictBody([
    ...
    fieldRules: {
        ...
        features: { type: 'array', maxItems: 50 },
        color_theme: { type: 'string', maxLength: 40 },
        is_popular: { type: 'boolean' },
        description: { type: 'string', maxLength: 4000 },
        discount_percent: { type: 'integer', min: 0, max: 100 },
        discount_valid_until: { type: 'string', pattern: /^\d{4}-\d{2}-\d{2}$/ },
        joining_fee: { type: 'number', min: 0, max: 1000000 },
        freeze_allowance_days: { type: 'integer', min: 0, max: 3650 },
        transfer_fee: { type: 'number', min: 0, max: 1000000 },
        access_hours: { type: 'string', maxLength: 500 },
        guest_passes: { type: 'integer', min: 0, max: 365 },
        renewal_policy: { type: 'string', maxLength: 500 },
        class_eligibility: { type: 'string', maxLength: 500 },
        advanced_rules: { type: 'object' },
    },
});
const planAdvancedRulesBody = strictBody([
    'joining_fee',
    'freeze_allowance_days',
    'transfer_fee',
    'access_hours',
...
const emptyPlanBody = strictBody([], { allowEmpty: true });
...
// CREATE A NEW PLAN
router.post('/add', planBody, async (req, res) => {
    try {
...
// DELETE PLAN
router.delete('/:id', emptyPlanBody, async (req, res) => {
    try {
        const { id } = req.params;
...
// UPDATE PLAN
router.put('/:id', planBody, async (req, res) => {
    try {
...
// --- ADVANCED RULES ---
router.put('/:id/advanced-rules', planAdvancedRulesBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
