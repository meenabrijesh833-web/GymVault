...
const { paymentSignatureMatches, recordPaymentSignatureRejection } = require('../utils/paymentSecurity');
...
const membershipActivationBody = strictBody(['member_id', 'plan_id', 'payment_id', 'payment_mode'], {
    requiredFields: ['member_id', 'plan_id'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        plan_id: { type: 'integer', min: 1 },
        payment_id: { type: 'string', maxLength: 120 },
        payment_mode: { type: 'string', enum: ['CASH', 'ONLINE'], caseInsensitive: true },
    },
});
const membershipOnlineOrderBody = strictBody(['member_id', 'plan_id'], {
    requiredFields: ['member_id', 'plan_id'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        plan_id: { type: 'integer', min: 1 },
...
const membershipPaymentLinkStatusBody = strictBody(['member_id', 'plan_id', 'payment_link_id'], {
    requiredFields: ['member_id', 'plan_id', 'payment_link_id'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        plan_id: { type: 'integer', min: 1 },
        payment_link_id: { type: 'string', minLength: 1, maxLength: 120 },
    },
});
const linkedAccountCreateBody = strictBody([
    'legal_business_name',
    'business_email',
    'business_phone',
    'city',
    ...
    requiredFields: ['legal_business_name', 'business_email', 'city', 'state', 'pincode'],
    fieldRules: {
        legal_business_name: { type: 'string', minLength: 2, maxLength: 120 },
        business_email: { type: 'string', format: 'email', maxLength: 120 },
        business_phone: { type: 'string', format: 'phone', maxLength: 30 },
        city: { type: 'string', minLength: 2, maxLength: 60 },
        state: { type: 'string', minLength: 2, maxLength: 40 },
        pincode: { type: 'string', pattern: /^\d{6}$/ },
    },
});
const linkedAccountSaveBody = strictBody(['account_id'], {
    requiredFields: ['account_id'],
    fieldRules: {
        account_id: { type: 'string', pattern: /^acc_[A-Za-z0-9]+$/, maxLength: 120 },
    },
});
const membershipOnlineVerificationBody = strictBody([
    'member_id',
    'plan_id',
    'razorpay_order_id',
    'razorpay_payment_id',
    ...
    requiredFields: ['member_id', 'plan_id', 'razorpay_order_id', 'razorpay_payment_id', 'razorpay_signature'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        plan_id: { type: 'integer', min: 1 },
        razorpay_order_id: { type: 'string', minLength: 1, maxLength: 120 },
        razorpay_payment_id: { type: 'string', minLength: 1, maxLength: 120 },
        razorpay_signature: { type: 'string', minLength: 1, maxLength: 256 },
    },
});
const emptyMembershipBody = strictBody([], { allowEmpty: true });
const membershipPlanBody = strictBody(['plan_name', 'duration_days', 'price'], {
    requiredFields: ['plan_name', 'duration_days', 'price'],
    fieldRules: {
        plan_name: { type: 'string', minLength: 2, maxLength: 120 },
        duration_days: { type: 'integer', min: 1, max: 3650 },
        price: { type: 'number', min: 0, max: 1000000 },
    },
});
const membershipIdBody = strictBody(['membership_id'], {
    requiredFields: ['membership_id'],
    fieldRules: { membership_id: { type: 'integer', min: 1 } },
});
const memberIdBody = strictBody(['member_id'], {
    requiredFields: ['member_id'],
    fieldRules: { member_id: { type: 'integer', min: 1 } },
});
const membershipFreezeBody = strictBody(['member_id', 'freeze_reason', 'freeze_end_date'], {
    requiredFields: ['member_id'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        freeze_reason: { type: 'string', maxLength: 500 },
        freeze_end_date: { type: 'string', maxLength: 60 },
    },
});
const membershipExtensionBody = strictBody(['member_id', 'days'], {
    requiredFields: ['member_id', 'days'],
    fieldRules: {
        member_id: { type: 'integer', min: 1 },
        days: { type: 'integer', min: 1, max: 3650 },
    },
});
const membershipGraceBody = strictBody(['grace_days'], {
    allowEmpty: true,
    fieldRules: { grace_days: { type: 'integer', min: 1, max: 90 } },
});
const membershipCancellationBody = strictBody(['cancellation_reason'], {
    allowEmpty: true,
    fieldRules: { cancellation_reason: { type: 'string', maxLength: 500 } },
});
const bulkMembershipFreezeBody = strictBody(['member_ids', 'freeze_reason'], {
    requiredFields: ['member_ids'],
    fieldRules: {
        member_ids: { type: 'array', minItems: 1, maxItems: 100 },
        freeze_reason: { type: 'string', maxLength: 500 },
    },
});
const bulkMembershipUnfreezeBody = strictBody(['member_ids'], {
    requiredFields: ['member_ids'],
    fieldRules: { member_ids: { type: 'array', minItems: 1, maxItems: 100 } },
});
...
// --- 1. ACTIVATE / RENEW ---
router.post('/activate', auth, saasMiddleware, requirePermission('payments:write'), membershipActivationBody, async (req, res) => {
    try {
...
router.post('/online/create-order', auth, saasMiddleware, requirePermission('payments:write'), membershipOnlineOrderBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const memberId = ensureInteger(req.body?.member_id, { field: 'member_id', required: true, min: 1 });
        const planId = ensureInteger(req.body?.plan_id, { field: 'plan_id', required: true, min: 1 });
...
router.post('/online/payment-link-status', auth, saasMiddleware, requirePermission('payments:write'), membershipPaymentLinkStatusBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const memberId = ensureInteger(req.body?.member_id, { field: 'member_id', required: true, min: 1 });
        const planId = ensureInteger(req.body?.plan_id, { field: 'plan_id', required: true, min: 1 });
...
router.post('/online/connect/disconnect', auth, saasMiddleware, requireOwner, emptyMembershipBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
        await pool.query(
            `UPDATE gyms
...
router.post('/online/linked-account/create', auth, saasMiddleware, requireOwner, linkedAccountCreateBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
        const gym_id = req.user.gym_id;
...
router.post('/online/linked-account/save', auth, saasMiddleware, requireOwner, linkedAccountSaveBody, async (req, res) => {
    try {
        await ensureMemberPaymentsSchema();
        const gym_id = req.user.gym_id;
...
router.post('/online/verify', auth, saasMiddleware, requirePermission('payments:write'), membershipOnlineVerificationBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const memberId = ensureInteger(req.body?.member_id, { field: 'member_id', required: true, min: 1 });
        const planId = ensureInteger(req.body?.plan_id, { field: 'plan_id', required: true, min: 1 });
        ...
        if (!paymentSignatureMatches(expectedSign, razorpay_signature)) {
            void recordPaymentSignatureRejection(req, 'member_membership');
...
// --- 3. CREATE PLAN ---
router.post('/plans', auth, saasMiddleware, requireOwner, membershipPlanBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
...
// --- 4. REMOVE PLAN ---
router.post('/remove-plan', auth, saasMiddleware, requirePermission('payments:write'), memberIdBody, async (req, res) => {
    let client;
    try {
...
// --- 6. RENEW ---
router.post('/renew', auth, saasMiddleware, requirePermission('payments:write'), membershipIdBody, async (req, res) => {
    try {
...
router.post('/freeze', auth, saasMiddleware, requirePermission('payments:write'), membershipFreezeBody, async (req, res) => {
    const gym_id = req.user.gym_id;
...
router.post('/unfreeze', auth, saasMiddleware, requirePermission('payments:write'), memberIdBody, async (req, res) => {
    const gym_id = req.user.gym_id;
...
// --- 7. QUICK EXTEND ---
router.post('/extend', auth, saasMiddleware, requirePermission('payments:write'), membershipExtensionBody, async (req, res) => {
    try {
        const member_id = ensureInteger(req.body?.member_id, { field: 'member_id', required: true, min: 1 });
...
router.post('/:id/grace', auth, saasMiddleware, requirePermission('members:write'), membershipGraceBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
...
// --- 9. CANCEL MEMBERSHIP ---
router.post('/:id/cancel', auth, saasMiddleware, requirePermission('members:write'), membershipCancellationBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const msId = ensureInteger(req.params.id, { field: 'membership id', required: true, min: 1 });
...
// --- BULK FREEZE ---
router.post('/bulk-freeze', auth, saasMiddleware, requirePermission('payments:write'), bulkMembershipFreezeBody, async (req, res) => {
    const gym_id = req.user.gym_id;
    try {
...
// --- BULK UNFREEZE ---
router.post('/bulk-unfreeze', auth, saasMiddleware, requirePermission('payments:write'), bulkMembershipUnfreezeBody, async (req, res) => {
    const gym_id = req.user.gym_id;
    try {
        const memberIds = req.body?.member_ids;
        if (!Array.isArray(memberIds) || memberIds.length === 0 || memberIds.length > 100) {
