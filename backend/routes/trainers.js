...
const trainerAssignmentBody = strictBody(['trainer_user_id', 'member_id', 'notes'], {
    requiredFields: ['trainer_user_id', 'member_id'],
    fieldRules: {
        trainer_user_id: { type: 'integer', min: 1 },
        member_id: { type: 'integer', min: 1 },
        notes: { type: 'string', maxLength: 500 },
    },
});
const trainerTaskBody = strictBody(['trainer_user_id', 'member_id', 'title', 'description', 'due_date'], {
    requiredFields: ['trainer_user_id', 'title'],
    fieldRules: {
        trainer_user_id: { type: 'integer', min: 1 },
        member_id: { type: 'integer', min: 1 },
        title: { type: 'string', minLength: 1, maxLength: 200 },
        description: { type: 'string', maxLength: 2000 },
        due_date: { maxLength: 60 },
    },
});
const trainerTaskStatusBody = strictBody(['status'], {
    requiredFields: ['status'],
    fieldRules: { status: { type: 'string', enum: ['PENDING', 'IN_PROGRESS', 'COMPLETED'], caseInsensitive: true } },
});
const emptyTrainerBody = strictBody([], { allowEmpty: true });
...
router.post('/assignments', auth, saasMiddleware, requirePermission('members:write'), trainerAssignmentBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
...
// --- REMOVE ASSIGNMENT ---
router.delete('/assignments/:id', auth, saasMiddleware, requirePermission('members:write'), emptyTrainerBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
...
router.post('/tasks', auth, saasMiddleware, requirePermission('members:write'), trainerTaskBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const trainerUserId = ensureInteger(req.body?.trainer_user_id, { field: 'trainer_user_id', required: true, min: 1 });
...
router.patch('/tasks/:id', auth, saasMiddleware, trainerTaskStatusBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
...
router.delete('/tasks/:id', auth, saasMiddleware, requirePermission('members:write'), emptyTrainerBody, async (req, res) => {
    try {
        const gym_id = req.user.gym_id;
        const taskId = ensureInteger(req.params.id, { field: 'task_id', required: true, min: 1 });
