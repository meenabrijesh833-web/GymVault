...
const classTypeBody = strictBody([
    'title',
    'category',
    'description',
    'trainer_name',
    ...
    fieldRules: {
        ...
        trainer_name: { type: 'string', maxLength: 120 },
        capacity: { type: 'integer', min: 1, max: 500 },
        duration_minutes: { type: 'integer', min: 5, max: 720 },
        location: { type: 'string', maxLength: 120 },
        branch_id: { type: 'string', maxLength: 60 },
        color_theme: { type: 'string', maxLength: 40 },
...
const classSessionFieldRules = {
    class_type_id: { type: 'integer', min: 1 },
    starts_at: { maxLength: 60 },
    ends_at: { maxLength: 60 },
    duration_minutes: { type: 'integer', min: 5, max: 720 },
    trainer_name: { type: 'string', maxLength: 120 },
    capacity: { type: 'integer', min: 1, max: 500 },
    status: { type: 'string', enum: ['SCHEDULED', 'CANCELLED', 'COMPLETED'], caseInsensitive: true },
    repeat_mode: { type: 'string', enum: ['DAILY', 'WEEKDAYS', 'WEEKLY', 'CUSTOM'], caseInsensitive: true },
    repeat_until: { type: 'string', minLength: 1, maxLength: 60 },
    repeat_days: { type: 'array', maxItems: 7 },
    notes: { type: 'string', maxLength: 1000 },
    branch_id: { type: 'string', maxLength: 60 },
};
const classSessionCreateBody = strictBody(classSessionFields, {
    requiredFields: ['class_type_id', 'starts_at'],
    fieldRules: classSessionFieldRules,
});
const classSessionRecurringBody = strictBody(classSessionFields, {
    requiredFields: ['class_type_id', 'starts_at', 'repeat_mode', 'repeat_until'],
    fieldRules: classSessionFieldRules,
});
const classSessionUpdateBody = strictBody(classSessionFields, { fieldRules: classSessionFieldRules });
const classBookingBody = strictBody(['member_id', 'notes', 'branch_id'], {
...
const optionalClassBranchBody = strictBody(['branch_id'], {
    allowEmpty: true,
...
router.post('/types', requirePermission('attendance:write'), classTypeBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.put('/types/:id', requirePermission('attendance:write'), classTypeBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.post('/sessions', requirePermission('attendance:write'), classSessionCreateBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.post('/sessions/recurring', requirePermission('attendance:write'), classSessionRecurringBody, async (req, res) => {
    let client;
    try {
        const gymId = getGymIdFromRequest(req);
        const classTypeId = ensureInteger(req.body?.class_type_id, { field: 'class_type_id', required: true, min: 1 });
...
router.put('/sessions/:id', requirePermission('attendance:write'), classSessionUpdateBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.post('/sessions/:id/bookings', requirePermission('attendance:write'), classBookingBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.delete('/sessions/:sessionId/bookings/:memberId', requirePermission('attendance:write'), optionalClassBranchBody, async (req, res) => {
    try {
        const gymId = getGymIdFromRequest(req);
...
router.post('/sessions/:sessionId/bookings/:memberId/check-in', requirePermission('attendance:write'), optionalClassBranchBody, async (req, res) => {
    let client;
