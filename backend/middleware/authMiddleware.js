...
module.exports = async (req, res, next) => {
    ...
    if (!token) {
        void recordSecurityEvent(req, {
            ...
            message: 'Owner or staff request rejected because authentication was missing.',
            statusCode: 401,
        });
    ...
    if (!SAFE_METHODS.has(req.method) && tokenSource === 'cookie') {
        void recordSecurityEvent(req, {
            ...
            message: 'Mutation rejected because only cookie authentication was supplied.',
            statusCode: 401,
    ...
    try {
        ...
        if (!Number.isInteger(userId) || !Number.isInteger(gymId)) {
            void recordSecurityEvent(req, {
                ...
                message: 'Authentication token claims were invalid.',
                statusCode: 401,
                metadata: { reason: 'invalid_claims', token_source: tokenSource },
            });
            return res.status(401).json({
                success: false,
                code: 'AUTH_INVALID',
        ...
        if (!session) {
            void recordSecurityEvent(req, {
                ...
                message: 'Authenticated identity did not match an active tenant session.',
                statusCode: 401,
                metadata: { reason: 'session_not_found', token_source: tokenSource },
            });
            return res.status(401).json({
                success: false,
                code: 'AUTH_INVALID',
        ...
        void recordSecurityEvent(req, {
            ...
            statusCode: 401,
            metadata: { reason: err?.name || 'verification_failed', token_source: tokenSource },
        });
        res.status(401).json({
            success: false,
            code: 'AUTH_INVALID',
