...
const checkExpirations = require('../operations/jobs/expiryCheck');
const { runAutomatedNotificationNudges } = require('../operations/jobs/notificationAutomation');
const { runRetentionMaintenance } = require('../operations/jobs/retentionMaintenance');
const { runDatabaseBackup } = require('../operations/jobs/databaseBackup');
const { runPayrollAutoPay } = require('../operations/jobs/payrollAutoPay');
...
const trustProxySetting = rawTrustProxySetting === true ? 1 : rawTrustProxySetting;
...
const buildRateLimitHandler = ({ windowMs, code, description, messagePrefix = 'Too many' }) => (req, res) => {
    ...
    const message = `${messagePrefix} ${description}. Retry in about ${retryAfterSeconds} seconds.`;

    void recordRuntimeEvent({
        eventType: 'RATE_LIMIT_BLOCK',
        severity: 'WARN',
        source: 'security',
        message: `${code}: ${req.method} ${req.originalUrl || req.path || '/'}`,
...
const buildScopedLimiter = ({ windowMs, productionMax, developmentMax, code, description }) => rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    handler: buildRateLimitHandler({ windowMs, code, description }),
});
...
const otpDispatchLimiter = buildScopedLimiter({
    windowMs: 15 * 60 * 1000,
    productionMax: 12,
    developmentMax: 120,
    code: 'OTP_DISPATCH_RATE_LIMITED',
    description: 'OTP requests',
});

const authDiscoveryLimiter = buildScopedLimiter({
    windowMs: 15 * 60 * 1000,
    productionMax: 60,
    developmentMax: 600,
    code: 'AUTH_DISCOVERY_RATE_LIMITED',
    description: 'account checks',
});

const ownerRegistrationLimiter = buildScopedLimiter({
    windowMs: 60 * 60 * 1000,
    productionMax: 12,
    developmentMax: 120,
    code: 'OWNER_REGISTRATION_RATE_LIMITED',
    description: 'owner registrations',
});

const attendanceWriteLimiter = buildScopedLimiter({
    windowMs: 15 * 60 * 1000,
    productionMax: 500,
    developmentMax: 5000,
    code: 'ATTENDANCE_WRITE_RATE_LIMITED',
    description: 'attendance writes',
});

const financialWriteLimiter = buildScopedLimiter({
    windowMs: 15 * 60 * 1000,
    productionMax: 120,
    developmentMax: 1200,
    code: 'FINANCIAL_WRITE_RATE_LIMITED',
    description: 'financial changes',
});

const clientErrorLimiter = buildScopedLimiter({
    windowMs: 15 * 60 * 1000,
    productionMax: 30,
    developmentMax: 300,
    code: 'CLIENT_ERROR_RATE_LIMITED',
    description: 'client error reports',
});
...
app.use('/api/auth/check-email', authDiscoveryLimiter);
app.use('/api/auth/check-phone', authDiscoveryLimiter);
app.use('/api/auth/signup/send-email-otp', otpDispatchLimiter);
app.use('/api/auth/signup/verify-email-otp', authLimiter);
app.use('/api/auth/register-owner', ownerRegistrationLimiter);
app.use('/api/auth/google/signup/complete', ownerRegistrationLimiter);
app.use('/api/auth/admin/send-otp', otpDispatchLimiter);
app.use('/api/auth/admin/verify-otp', authLimiter);
app.use('/api/auth/admin/send-email-otp', otpDispatchLimiter);
app.use('/api/auth/admin/verify-email-otp', authLimiter);
app.use('/api/superadmin/login', authLimiter);
app.use('/api/auth/member/profiles', authDiscoveryLimiter);
app.use('/api/auth/member/send-otp', otpDispatchLimiter);
app.use('/api/auth/member/verify-otp', authLimiter);
app.use('/api/auth/password-reset/request', otpDispatchLimiter);
...
app.use('/api/support/client-errors', limitMethods(['POST'], clientErrorLimiter));
app.use('/api/attendance/checkin', limitMethods(['POST'], attendanceWriteLimiter));
app.use('/api/attendance/member/checkin', limitMethods(['POST'], attendanceWriteLimiter));
app.use('/api/attendance/rfid/event', limitMethods(['POST'], rfidEventLimiter));
app.use('/api/payments', limitMethods(['POST', 'PUT', 'PATCH', 'DELETE'], financialWriteLimiter));
app.use('/api/memberships', limitMethods(['POST', 'PUT', 'PATCH', 'DELETE'], financialWriteLimiter));
app.use('/api/billing', limitMethods(['POST', 'PUT', 'PATCH', 'DELETE'], financialWriteLimiter));
app.use('/api/finance', limitMethods(['POST', 'PUT', 'PATCH', 'DELETE'], financialWriteLimiter));
