...
│  │  └── ../operations/jobs/ (scheduled jobs)        │    │
│  └─────────────────────────────────────────────────┘    │
└───────────────────────┬─────────────────────────────────┘
                        │ SQL queries
                        ▼
...
- **Auto-migrations** — `backend/config/db.js` runs migrations on every boot from `database/init.sql`
...
**Endpoints involved:** `backend/routes/payments.js`, `backend/routes/memberships.js`, `backend/routes/member.js`, `backend/routes/billing.js`
...
**Configuration:** `backend/utils/msg91.js`, `backend/utils/whatsappDelivery.js`
...
**Configuration:** `backend/routes/auth.js` (OAuth redirect and callback handlers)
...
**Configuration:** `backend/utils/email.js` — supports any SMTP provider (Gmail, SendGrid, AWS SES, etc.)
...
**Configuration:** `backend/routes/push.js`
...
**Configuration:** `backend/utils/cache.js`
...
GymVault uses **PostgreSQL** hosted on **Supabase**. The schema is managed entirely through code — `backend/config/db.js` applies `database/init.sql` and idempotent migrations on server boot. No external migration tool is needed.
...
- **Encrypted sensitive data** — Razorpay secrets, bank details stored encrypted (`backend/utils/secretCrypto.js`)
...
- The `retentionMaintenance` job archives old records using environment-configurable limits; attendance and RFID events both default to 365 days
- Runtime events default to 60-day archival and are also capped at 5,000 live rows
- `database/tenantTablePolicy.js` is the required physical-table access inventory; unclassified public tables fail backend startup
