...
Gym owners pay GymVault for plans and add-ons through the main Razorpay account. The backend creates and verifies Razorpay orders in `backend/routes/billing.js` using:
...
Tenant database access uses the non-login `gymvault_app` role by default. Startup compares every physical table in the `public` schema with `database/tenantTablePolicy.js`, revokes accumulated runtime table privileges, and restores only the declared grants and RLS policies. Startup fails if a table is unclassified, an expected table is missing, a direct tenant table lacks `gym_id`, or an undeclared RLS policy exists. Update the inventory and add an explicit policy implementation in the same change whenever a physical table is added.

`npm run check:tenant-rls` verifies the live physical-table inventory, exact runtime grants, exact RLS policy set, and rollback-only cross-tenant read/write fixtures. `npm run check:tenant-context` verifies pooled queries and acquired clients receive and clear the tenant role and `app.current_gym_id` context.

## Runtime security events and retention

Runtime, attack, provider, handled 5xx, process, and database-denial events are stored in `system_runtime_events`. Metadata, messages, and stacks are recursively bounded and redact credentials, tokens, personal contact data, JWTs, and the application root path. Payment signature values, provider secrets, SQL statements, and SQL parameters must never be added to event metadata.

Retention has two independent bounds:

- `RUNTIME_EVENT_MAX_ROWS` defaults to 5,000. Event writes schedule failure-isolated trimming of older rows above that limit.
- `RUNTIME_EVENT_RETENTION_DAYS` defaults to 60 and cannot be lower than 14. The retention job runs on startup and every 12 hours on the designated background-job worker, moving expired events into `operational_archives` in bounded batches.

`DATA_ARCHIVE_ENABLED=false` disables time-based archival, but does not disable row-count trimming. `ARCHIVE_BATCH_SIZE` defaults to 2,000 and `ARCHIVE_MAX_BATCH_PASSES` defaults to 10 per source table and maintenance run. Archived runtime events remain admin-only; the tenant runtime role cannot read `operational_archives`.

Investigate repeated `SECURITY_PAYMENT_SIGNATURE_INVALID`, `SECURITY_DATABASE_ACCESS_DENIED`, `SECURITY_WEBHOOK_REJECTED`, `REQUEST_SCHEMA_REJECT`, and `REQUEST_ERROR` events. Run `npm run check:security-events` after telemetry or database-access changes; its fixtures and generated events are self-cleaning.
