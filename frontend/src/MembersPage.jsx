...
import { buildWhatsAppConversationUrl } from './utils/externalNavigation';
...
const WAIVER_CANVAS_BACKGROUND = '#0f172a';
const WAIVER_CANVAS_BACKGROUND_RGB = { r: 15, g: 23, b: 42 };
const WAIVER_CANVAS_STROKE = '#f8fafc';
...
const readWhatsAppStarterAckMarker = (memberId) => {
  if (!memberId || typeof window === 'undefined') return '';
  try {
    return String(window.localStorage.getItem(getWhatsAppStarterAckKey(memberId)) || '');
  } catch (_err) {
    return '';
  }
};

const writeWhatsAppStarterAckMarker = (memberId, marker) => {
  if (!memberId || !marker || typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(getWhatsAppStarterAckKey(memberId), String(marker));
  } catch (_err) {
...
const MembersPage = ({ appRuntime, defaultFilter = 'All', focusMemberId = null, focusAction = null, onFocusHandled, isActive = true }) => {
  ...
  const [signedWaiverData, setSignedWaiverData] = useState(null);
  ...
  const handleSignWaiver = async () => {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
      if (
        pixels[i] !== WAIVER_CANVAS_BACKGROUND_RGB.r
        || pixels[i + 1] !== WAIVER_CANVAS_BACKGROUND_RGB.g
        || pixels[i + 2] !== WAIVER_CANVAS_BACKGROUND_RGB.b
        || pixels[i + 3] !== 255
      ) {
        hasDrawing = true;
        break;
  ...
  const initWaiverCanvas = useCallback((canvas) => {
    ...
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = WAIVER_CANVAS_BACKGROUND;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.lineWidth = 2.5;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.strokeStyle = WAIVER_CANVAS_STROKE;
    const getPos = (e) => {
...
<body>
  ...
  const activeWhatsAppStarterSuppression = useMemo(() => {
    const suppression = selectedMember?.whatsapp_meta_suppression;
    return suppression && suppression.active ? suppression : null;
  }, [selectedMember?.whatsapp_meta_suppression]);

  const activeWhatsAppStarterMarker = useMemo(() => String(activeWhatsAppStarterSuppression?.last_failed_at || ''), [activeWhatsAppStarterSuppression?.last_failed_at]);

  const showWhatsAppStarterCard = Boolean(activeWhatsAppStarterSuppression && activeWhatsAppStarterMarker && whatsappStarterAckMarker !== activeWhatsAppStarterMarker);

  useEffect(() => {
    if (!selectedMember?.id || !activeWhatsAppStarterMarker) {
      setWhatsAppStarterAckMarker('');
      return;
    }

    setWhatsAppStarterAckMarker(readWhatsAppStarterAckMarker(selectedMember.id));
  }, [activeWhatsAppStarterMarker, selectedMember?.id]);

  useEffect(() => {
    if (!showWhatsAppStarterCard || !token) {
  ...
  }, [showWhatsAppStarterCard, token]);

  const memberWhatsAppStarterMessage = useMemo(() => {
    const fullName = String(selectedMember?.full_name || 'Member').trim() || 'Member';
    return `Hi, this is ${fullName}. I am messaging from my phone so ${gymWhatsAppStarter.displayName || 'the gym'} can send my membership updates here.`;
  }, [gymWhatsAppStarter.displayName, selectedMember?.full_name]);
  ...
  const acknowledgeWhatsAppStarterPrompt = useCallback(() => {
    const memberId = Number(selectedMember?.id || 0);
    if (!memberId || !activeWhatsAppStarterMarker) return;
    writeWhatsAppStarterAckMarker(memberId, activeWhatsAppStarterMarker);
    setWhatsAppStarterAckMarker(activeWhatsAppStarterMarker);
  }, [activeWhatsAppStarterMarker, selectedMember?.id]);
  ...
  const handleCopyMemberWhatsAppStarterLink = useCallback(async () => {
    ...
    if (copied) {
      acknowledgeWhatsAppStarterPrompt();
      toast?.('Member WhatsApp link copied. Open it from the member phone or let the member scan the QR once.', 'success');
      return;
    }

    toast?.('Copy failed on this device. Long-press and copy it manually.', 'warning');
  }, [acknowledgeWhatsAppStarterPrompt, memberWhatsAppStarterUrl, toast]);
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3 sm:gap-5 p-1 sm:p-2 relative members-page-shell">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-slate-900 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-2 text-[11px] font-bold text-slate-800">
                      {activeWhatsAppStarterSuppression?.prompt_message || 'After the member sends that first message, retry the reminder later for this member.'}
                    </p>
                    {gymWhatsAppStarterLoading ? (
                      <p className="mt-2 text-[11px] font-bold text-slate-600">Loading your connected gym WhatsApp...</p>
                    ) : gymWhatsAppStarter.ready ? (
                      <p className="mt-2 text-[11px] font-bold text-emerald-700">Member chat target: {gymWhatsAppStarter.displayName || 'your gym WhatsApp'}.</p>
                    ) : (
                      <p className="mt-2 text-[11px] font-bold text-rose-700">Connect the gym business WhatsApp number in Settings first.</p>
                    )}
                  </div>
                  {memberWhatsAppStarterUrl ? (
                    <div className="mx-auto rounded-[20px] border border-slate-300 bg-white p-2.5 shadow-sm sm:mx-0">
                      <QRCodeCanvas value={memberWhatsAppStarterUrl} size={94} includeMargin level="M" bgColor="#ffffff" fgColor="#111827" />
                      <p className="mt-2 text-center text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Scan on member phone</p>
                    </div>
                  ) : null}
                </div>
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 py-2.5 text-xs font-black uppercase tracking-[0.18em] text-white transition-all hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <Copy size={14} /> Copy Member Link
                  </button>
                  <button
                    type="button"
                    onClick={acknowledgeWhatsAppStarterPrompt}
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-xs font-black uppercase tracking-[0.18em] text-slate-700 transition-all hover:bg-slate-100"
                  >
                    <X size={14} /> Hide For Now
                  </button>
                </div>
              </div>
            ) : null}
        ...
        <div className="app-modal-shell z-50 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="app-modal-panel bg-white rounded-[28px] w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-150">
            ...
            <form onSubmit={handleAddMember} className="app-modal-scroll p-6 space-y-4">
              ...
              {canWritePayments && (
                <div className="rounded-2xl border border-slate-200 bg-slate-900 p-2.5 shadow-inner">
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => { setAddSelectedPlanId(''); setAddFormData((prev) => ({ ...prev, onboarding_mode: 'FRESH', migration_plan_id: '', migration_membership_start_date: '', migration_membership_end_date: '', migration_last_visit_date: '' })); }} className={`rounded-xl px-3 py-2 text-xs font-black uppercase tracking-wide transition-all ${!isExistingMemberOnboarding ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-900/30' : 'border border-slate-700 bg-slate-800 text-slate-300 hover:border-slate-600 hover:text-white'}`}>Fresh Member</button>
                    <button type="button" onClick={() => { setAddSelectedPlanId(''); setAddFormData((prev) => ({ ...prev, onboarding_mode: 'EXISTING' })); }} className={`rounded-xl px-3 py-2 text-xs font-black uppercase tracking-wide transition-all ${isExistingMemberOnboarding ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-900/30' : 'border border-slate-700 bg-slate-800 text-slate-300 hover:border-slate-600 hover:text-white'}`}>Existing Active Member</button>
                  </div>
                  {isExistingMemberOnboarding && <p className="mt-2 px-1 text-[10px] font-medium leading-relaxed text-slate-300">Imports the member's current live plan without collecting a new payment.</p>}
                </div>
              )}
      ...
      {showWaiverModal && selectedMember && typeof document !== 'undefined' && createPortal(
        <div className="app-modal-shell z-[210] bg-slate-950/74 backdrop-blur-md" onClick={closeWaiverModal}>
          <div
            role="dialog"
            aria-modal="true"
            aria-label={signedWaiverData ? 'Signed liability waiver' : 'Sign liability waiver'}
            className="app-modal-panel w-full overflow-hidden rounded-[28px] border border-white/10 text-white shadow-2xl animate-in zoom-in-95"
            style={{ background: 'linear-gradient(180deg, #182234 0%, #0f172a 100%)' }}
            onClick={(event) => event.stopPropagation()}
          >
            {signedWaiverData ? (
              <>
                <div className="flex items-start justify-between gap-3 px-5 py-4" style={{ background: 'linear-gradient(135deg, #047857 0%, #065f46 100%)' }}>
                  <div>
                    <div className="mb-1 flex items-center gap-2">
                      <CheckCircle size={16} className="text-emerald-100" />
                      <span className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-100/90">Waiver Signed Successfully</span>
                    </div>
                    <h2 className="text-lg font-black">Signed Liability Waiver</h2>
                    <p className="mt-0.5 text-xs text-emerald-100/80">{signedWaiverData.memberName}</p>
                  </div>
                  <button onClick={closeWaiverModal} className="rounded-full p-2 text-white/80 transition-colors hover:bg-white/10 hover:text-white"><X size={18} /></button>
                </div>
                <div className="app-modal-scroll space-y-4 px-4 py-4 sm:px-5 sm:py-5">
                  <div className="space-y-4 rounded-[24px] border border-emerald-400/15 bg-slate-950/60 p-4">
                    <div className="border-b border-emerald-400/10 pb-3 text-center">
                      <p className="mb-1 text-[10px] font-black uppercase tracking-[0.24em] text-emerald-300">Official Signed Document</p>
                      <h3 className="text-sm font-black tracking-wide text-white">GYM LIABILITY WAIVER AND RELEASE</h3>
                    </div>
                    <div className="flex items-start justify-between gap-3 text-[11px] text-slate-300">
                      <div className="min-w-0">
                        <p className="font-semibold uppercase tracking-wide text-slate-400">Member</p>
                        <p className="truncate text-sm font-black text-white">{signedWaiverData.memberName}</p>
                      </div>
                      <p className="shrink-0 text-right font-semibold text-slate-300">
                        {signedWaiverData.signedAt.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </p>
                    </div>
                    <div className="max-h-52 overflow-y-auto rounded-2xl border border-white/10 bg-slate-900/90 p-3 text-[12px] leading-6 text-slate-100 whitespace-pre-wrap">
                      {signedWaiverData.waiverText || WAIVER_TERMS_TEXT}
                    </div>
                    <div className="space-y-2 border-t border-white/10 pt-3">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Member Signature</p>
                      <div className="flex min-h-[96px] items-center justify-center rounded-2xl border border-white/10 bg-slate-950 px-4 py-3">
                        {signedWaiverData.signature ? (
                          <img src={signedWaiverData.signature} alt="Member signature" className="max-h-20 w-auto object-contain" />
                        ) : (
                          <p className="text-xs font-semibold italic text-slate-500">Signature not available</p>
                        )}
                      </div>
                      <p className="text-center text-[11px] leading-relaxed text-slate-400">
                        Signed digitally on{' '}
                        {signedWaiverData.signedAt.toLocaleString('en-IN', {
                  ...
                  <button onClick={closeWaiverModal} className="w-full rounded-2xl bg-emerald-600 py-3.5 text-sm font-black text-white transition-all hover:bg-emerald-700">
                    Done
                  </button>
                </div>
              </>
              ...
              <>
                <div className="flex items-center justify-between gap-4 px-5 py-4 text-white" style={{ background: 'linear-gradient(135deg, #4338ca 0%, #6d28d9 100%)' }}>
                  <div>
                    <h2 className="text-base font-black tracking-tight">Sign Liability Waiver</h2>
                    <p className="mt-0.5 text-xs text-indigo-100/80">{selectedMember.full_name}</p>
                  </div>
                  <button onClick={closeWaiverModal} className="rounded-full p-2 text-white/80 transition-colors hover:bg-white/10 hover:text-white"><X size={18} /></button>
                </div>
                <div className="app-modal-scroll space-y-4 px-4 py-4 sm:px-5 sm:py-5">
                  <div className="max-h-60 overflow-y-auto rounded-[24px] border border-white/10 bg-slate-900/90 p-4 text-[12px] font-medium leading-6 text-slate-100 whitespace-pre-wrap">
                    {WAIVER_TERMS_TEXT}
                  </div>
                  <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
                    <input type="checkbox" checked={waiverAgreed} onChange={(e) => setWaiverAgreed(e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0 rounded border-slate-600 bg-slate-900 text-indigo-500 focus:ring-indigo-500" />
                    <span className="text-sm font-bold leading-5 text-slate-100">I have read and agree to all terms and conditions above</span>
                  </label>
                  <div>
                    <div className="mb-1.5 flex items-center justify-between">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Signature</p>
                      <button type="button" onClick={clearWaiverSignature} className="text-[11px] font-bold text-indigo-300 transition-colors hover:text-indigo-200">Clear</button>
                    </div>
                    <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-inner shadow-black/20">
                      <canvas
                        ref={initWaiverCanvas}
                        width={400}
                        height={150}
                        className="block h-[140px] w-full cursor-crosshair touch-none"
                        style={{ background: WAIVER_CANVAS_BACKGROUND }}
                      />
                    </div>
                    <p className="mt-1.5 text-[11px] text-slate-400">Draw your signature above using finger or mouse.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={closeWaiverModal} className="rounded-2xl bg-white/5 py-3 text-sm font-black text-slate-200 transition-all hover:bg-white/10">Cancel</button>
                    <button onClick={handleSignWaiver} disabled={!waiverAgreed || waiverSigning} className="flex items-center justify-center gap-1.5 rounded-2xl bg-indigo-600 py-3 text-sm font-black text-white transition-all hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">
                      {waiverSigning ? <RefreshCw size={14} className="animate-spin" /> : <CheckCircle size={14} />}
